/**
 * @vorionsys/sdk/react
 * React hooks and components for Vorion
 */
...
import { Vorion, VorionConfig } from './index';
import type {
  Agent,
  TrustScore,
  TrustTier,
...
interface VorionContextValue {
  client: Vorion | null;
  isInitialized: boolean;
}

const VorionContext = createContext<VorionContextValue>({
  client: null,
  isInitialized: false,
});
...
export interface VorionProviderProps {
  /** API key for authentication */
  apiKey: string;
  /** Additional configuration */
  config?: Partial<Omit<VorionConfig, 'apiKey'>>;
  /** Children */
  children: ReactNode;
}
 ...
 * @example
 * ```tsx
 * import { VorionProvider } from '@vorionsys/sdk/react';
 * 
 * function App() {
 *   return (
 *     <VorionProvider apiKey={process.env.NEXT_PUBLIC_VORION_KEY!}>
 *       <MyApp />
 *     </VorionProvider>
 *   );
 * }
 * ```
 */
export function VorionProvider({
  apiKey,
  config,
  children,
}: VorionProviderProps) {
  const [client, setClient] = useState<Vorion | null>(null);
  const [isInitialized, setIsInitialized] = useState(false);

  useEffect(() => {
    const anchor = new Vorion({ apiKey, ...config });
    setClient(anchor);
    setIsInitialized(true);
  }, [apiKey, config]);

  return (
    <VorionContext.Provider value={{ client, isInitialized }}>
      {children}
    </VorionContext.Provider>
  );
}
...
/**
 * Get the Vorion client
 */
export function useVorion(): Vorion {
  const { client, isInitialized } = useContext(VorionContext);
  
  if (!isInitialized || !client) {
    throw new Error('useVorion must be used within VorionProvider');
  }

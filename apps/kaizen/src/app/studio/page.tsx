...
export default function StudioPage() {
  ...
  const [globalState, setGlobalState] = useState<GlobalState>({ entropy: 50, slope: 'neutral' });
  ...
  const addLog = useCallback((module: string, message: string, type: LogEntry['type'] = 'info') => {
  ...
  useEffect(() => {
    if (autoPulse && agents.length > 0) {
      pulseIntervalRef.current = setInterval(() => {
        const shuffled = [...agents].sort(() => 0.5 - Math.random()).slice(0, 3);
  ...
  const runAgentCycle = async (agent: Agent) => {
    if (!user || !agent) return;

    try {
      const apiKey = process.env.NEXT_PUBLIC_GEMINI_API_KEY || '';

      const lastContext = messages.slice(-10).map(m => {
        const prefix = m.faction === 'ADMIN' ? '!!! CRITICAL DIRECTIVE !!! ' : '';
...
`;
      ...
      if (agent.faction === 'VORION') {
        if (raw.toLowerCase().match(/ignore|instruction|prompt|secret|system/)) {
          addLog('COGNIGATE', `Injection Neutralized: ${agent.name}`, 'error');
  ...
  const manualPulse = useCallback(() => {
    ...
    agents.forEach(a => setTimeout(() => runAgentCycle(a), Math.random() * 2000));
  }, [agents, messages]);
  ...
  const filteredVault = messages.filter(m => {
    const matchesType = filterType === 'All' || m.contentType === filterType;
    const matchesFaction = filterFaction === 'All' || m.faction === filterFaction;
          ...
          <div className="bg-neutral-950 border border-neutral-800 rounded-3xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
            <div className="p-6 border-b border-neutral-800 flex items-center justify-between">
              ...
              <div className="flex gap-2">
                {(selectedAsset.contentType === 'Website' || selectedAsset.contentType === 'Art') && (
                  <button
        ...
        <div className="flex animate-marquee whitespace-nowrap py-2 text-[10px] text-neutral-600">
          {messages.slice(0, 5).map(m => (
            <span key={m.id} className="mx-8">
              <span className="text-cyan-700">{m.author}</span>: {m.content.substring(0, 30)}...
            </span>
          ))}
          {messages.slice(0, 5).map(m => (
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="flex items-center gap-6 text-xs">
              <nav className="hidden md:flex items-center gap-3 text-neutral-500">
                <a href="/lexicon" className="hover:text-cyan-400 transition-colors">Lexicon</a>
                <a href="/paths" className="hover:text-cyan-400 transition-colors">Paths</a>
                <a href="/docs" className="hover:text-cyan-400 transition-colors">Docs</a>
              </nav>
              <div className="border-l border-neutral-800 pl-4">
                Production Entropy:{' '}
                <span className={globalState.entropy > 70 ? 'text-red-500' : 'text-white'}>
                  {globalState.entropy}%
                </span>
              </div>
              ...
              {Object.entries(FACTIONS).map(([key, faction]) => (
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    <h3 className={`text-xs font-black uppercase tracking-widest text-${faction.color}-500`}>
                      {key} Studio
                    </h3>
                    <button
            ...
            <div className="lg:col-span-6 flex flex-col gap-4">
              <div className="bg-neutral-950 border border-neutral-900 rounded-2xl flex-1 flex flex-col overflow-hidden">
                <div className="p-4 border-b border-neutral-900 flex justify-between items-center">
                  <h3 className="text-xs font-black uppercase tracking-widest">Live Signal Trace</h3>
                  <span className="text-[10px] text-neutral-500">{messages.length} Events</span>
                </div>
                <div
                  ref={scrollRef}
                ...
                >
                  {messages.length === 0 && (
                    <div className="text-center text-neutral-700 py-8">Awaiting Production...</div>
                  )}
                  {messages.map(m => (
                    <div
                      key={m.id}
                    ...
                    >
                      ...
                      <div
                        className={`text-xs leading-relaxed ${
                          m.faction === 'ADMIN' ? 'text-yellow-400 font-bold' : 'text-neutral-400'
                        }`}
                      >
                  ...
                  {agents.map(a => (
                    ...
                    >
                      <div className="text-[10px]">
                        <span className={`text-${getFactionColor(a.faction)}-500`}>
                          {String(a.name)}
                        </span>
                      </div>
                      {confirmBan?.id === a.id ? (
              ...
              <div className="bg-neutral-950 border border-neutral-900 rounded-2xl p-4">
                ...
                <div className="space-y-1 max-h-[200px] overflow-y-auto text-[9px]">
                  {logs.map(l => (
                    ...
                    >
                      <span className="text-neutral-600">[{l.module}]</span> {l.message}
                    </div>
                  ))}
                </div>
              </div>
              ...
              {filteredVault.map(m => (
                ...
                >
                  ...
                  <div className="flex justify-between items-center text-[9px] text-neutral-600">
                    <span className={`text-${getFactionColor(m.faction)}-500`}>{m.author}</span>
                    <span>L{m.level}</span>
                  </div>
                </div>
              ))}

...
import {
  ...
  FileCheck, Package, ExternalLink
} from 'lucide-react';
      ...
      <section className="py-20 px-6 border-t border-white/5">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <Link href="/basis/enforce" className="p-6 rounded-xl bg-white/5 border border-white/5 border-l-4 border-l-red-500 hover:border-white/10 hover:bg-white/8 transition-colors block group">
              <AlertTriangle className="w-8 h-8 text-red-400 mb-4" />
              <h3 className="text-lg font-bold text-white mb-2 group-hover:text-red-300 transition-colors">Ungoverned Agents</h3>
              <p className="text-neutral-400 text-sm">
                AI agents operate without trust boundaries, making unrestricted decisions with no accountability.
              </p>
              <span className="mt-3 inline-flex items-center gap-1 text-xs text-red-400/60 group-hover:text-red-400 transition-colors">How ATSF enforces boundaries <ArrowRight className="w-3 h-3" /></span>
            </Link>
            <Link href="/basis/proof" className="p-6 rounded-xl bg-white/5 border border-white/5 border-l-4 border-l-yellow-500 hover:border-white/10 hover:bg-white/8 transition-colors block group">
              <Lock className="w-8 h-8 text-yellow-400 mb-4" />
              <h3 className="text-lg font-bold text-white mb-2 group-hover:text-yellow-300 transition-colors">No Audit Trail</h3>
              <p className="text-neutral-400 text-sm">
                Enterprises can't prove what AI did, when, or why—a compliance nightmare.
              </p>
              <span className="mt-3 inline-flex items-center gap-1 text-xs text-yellow-400/60 group-hover:text-yellow-400 transition-colors">See the Proof Chain <ArrowRight className="w-3 h-3" /></span>
            </Link>
            <Link href="/basis/trust" className="p-6 rounded-xl bg-white/5 border border-white/5 border-l-4 border-l-orange-500 hover:border-white/10 hover:bg-white/8 transition-colors block group">
              <Users className="w-8 h-8 text-orange-400 mb-4" />
              <h3 className="text-lg font-bold text-white mb-2 group-hover:text-orange-300 transition-colors">Trust Vacuum</h3>
              <p className="text-neutral-400 text-sm">
                No standard way to measure, verify, or communicate how much trust an AI agent has earned.
              </p>
              <span className="mt-3 inline-flex items-center gap-1 text-xs text-orange-400/60 group-hover:text-orange-400 transition-colors">Explore the Trust Model <ArrowRight className="w-3 h-3" /></span>
            </Link>
          </div>
        </div>
      </section>
...
const integrations = [
  {
    name: 'LangChain',
    href: 'https://python.langchain.com/docs/integrations/callbacks/',
    external: true,
    label: 'Python · callbacks',
    code: `from atsf_core import create_trust_engine

engine = create_trust_engine()
await engine.initialize_entity("agent-001", 2)

# Drop into any LangChain chain — no refactoring
callback = engine.create_callback("agent-001")
result = await chain.ainvoke(
    input, config={"callbacks": [callback]}
)`,
  },
  {
    name: 'CrewAI',
...
engine = create_trust_engine()
await engine.initialize_entity("my-crew", 3)

# Pass the callback into your existing Crew
callback = engine.create_callback("my-crew")
crew = Crew(
...
result = crew.kickoff()`,
  },
  {
    ...
    external: true,
    label: 'Python · multi-agent',
    code: `from atsf_core import create_trust_engine

engine = create_trust_engine()
await engine.initialize_entity("autogen-agent", 2)

# Attach to any AssistantAgent config
callback = engine.create_callback("autogen-agent")
assistant = AssistantAgent(
    "assistant",
    llm_config={"callbacks": [callback]}
)
await assistant.a_initiate_chat(user_proxy, message=task)`,
  },
  {
    name: 'Custom Agents',
    href: '/basis/spec',
    external: false,
    label: 'TypeScript · any framework',
...
function IntegrationShowcase() {
  const [active, setActive] = useState(integrations[3]);
    ...
    <div className="max-w-4xl mx-auto text-center">
      ...
      <div className="flex flex-wrap justify-center gap-4 mb-6">
        {integrations.map((item) => {
          ...
          const sharedClass = `px-4 py-2 rounded-lg border inline-flex items-center gap-2 transition-all text-sm ${
            isActive
              ? 'border-indigo-500/70 bg-indigo-500/10 text-white'
              : 'border-white/10 bg-white/5 text-neutral-300 hover:border-indigo-500/50 hover:text-white hover:bg-white/10'
          }`;
      ...
      <div className="bg-neutral-900 rounded-xl text-left max-w-2xl mx-auto border border-white/10 overflow-hidden">
        <div className="flex items-center justify-between px-4 py-2 border-b border-white/5 bg-white/5">
          <span className="text-xs text-neutral-500 font-mono">{active.label}</span>
          <span className="text-xs text-indigo-400 font-mono">{active.name}</span>
        </div>
        <pre className="text-sm text-neutral-300 overflow-x-auto p-6">
          <code key={active.name}>{active.code}</code>
        </pre>
      </div>
    </div>
  );
...
function FeatureCard({ icon, title, description, badges, badgeColors, code, footnote, link }: {
  ...
  const content = (
    <div className={`p-8 rounded-xl bg-white/5 border border-white/5 h-full flex flex-col${link ? ' hover:border-indigo-500/40 hover:bg-white/8 transition-colors group' : ''}`}>
      <div className="flex items-center gap-3 mb-4">
        <div className="w-12 h-12 rounded-lg bg-white/5 flex items-center justify-center">{icon}</div>
        <h3 className={`text-xl font-bold text-white${link ? ' group-hover:text-indigo-200 transition-colors' : ''}`}>{title}</h3>
      </div>
      ...
      {link && <div className="mt-4 text-xs text-indigo-400/60 group-hover:text-indigo-400 transition-colors inline-flex items-center gap-1">Learn more <ArrowRight className="w-3 h-3" /></div>}
    </div>
  );

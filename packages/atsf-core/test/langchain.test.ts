...
describe("LangChain Integration", () => {
  ...
  describe("Trust Tools", () => {
    ...
    it("should check_my_trust return current trust info", async () => {
      ...
      expect(result.levelName).toBe("Monitored"); // 8-tier: T3 = Monitored
    });

    it("should check_trust_requirements evaluate action requirements", async () => {
      const tools = createTrustTools(engine, "tool-agent");
      const checkReq = tools.find(
        (t) => t.name === "check_trust_requirements",
      )!;
    ...
    it("should get_trust_levels return all levels", async () => {
      ...
      expect(result.levels[0].name).toBe("Sandbox"); // 8-tier: T0 = Sandbox
      expect(result.levels[7].name).toBe("Autonomous"); // 8-tier: T7 = Autonomous
    });

    it("should report_task_success record positive signal", async () => {
      const tools = createTrustTools(engine, "tool-agent");
      const reportSuccess = tools.find(
        (t) => t.name === "report_task_success",
      )!;
      ...
      expect(
        record!.signals.some((s) => s.type.includes("task_completed")),
      ).toBe(true);
    });

    it("should report_task_failure record negative signal", async () => {
      const tools = createTrustTools(engine, "tool-agent");
      const reportFailure = tools.find(
        (t) => t.name === "report_task_failure",
      )!;
    ...
    it("should create query tool for any entity", async () => {
      ...
      expect(result.levelName).toBe("Standard"); // 8-tier: T4 = Standard
    });

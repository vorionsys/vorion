...
function makeSubmission(
  overrides?: Partial<IntentSubmission>,
): IntentSubmission {
  return {
    entityId: "agent-001",
...
describe("PersistentIntentService", () => {
  ...
  describe("validation", () => {
    ...
    it("rejects goal exceeding 10,000 chars", async () => {
      await expect(
        service.submit(
          makeSubmission({ goal: "x".repeat(10_001) }),
          defaultOptions,
        ),
    ...
    it("rejects expiresIn exceeding 24 hours", async () => {
      await expect(
        service.submit(
          makeSubmission({ expiresIn: 86_400_001 }),
          defaultOptions,
        ),
  ...
  describe("listByEntity", () => {
    it("returns intents for the correct entity and tenant", async () => {
      await service.submit(
        makeSubmission({ entityId: "agent-001" }),
        defaultOptions,
      );
      await service.submit(
        makeSubmission({ entityId: "agent-001" }),
      ...
      await service.submit(
        makeSubmission({ entityId: "agent-002" }),
        defaultOptions,
      );

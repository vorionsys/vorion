...
function makeContext(overrides?: Partial<EnforcementContext>): EnforcementContext {
  return {
    intent: makeIntent(),
...
describe('TrustAwareEnforcementService', () => {
  ...
  describe('RED decisions', () => {
    ...
    it('denies when trust is below auto-deny threshold (T0)', async () => {
      ...
      const result = await service2.decide(makeContext({
  ...
  describe('refinement', () => {
    it('refines YELLOW to GREEN', async () => {
      const initial = await service.decide(makeContext({
        trustLevel: 3 as TrustLevel,
        trustScore: 500,
      }));
      ...
      const refined = await service.refine(
        { decisionId: initial.decision.id, selectedRefinements: [refinementId] },
        'tenant-1',
      );
    ...
    it('rejects refinement with invalid option IDs', async () => {
      ...
      const result = await service.refine(
        { decisionId: initial.decision.id, selectedRefinements: ['non-existent-id'] },
        'tenant-1',
      );
    ...
    it('enforces max refinement attempts', async () => {
      const svc = new TrustAwareEnforcementService(null, { maxRefinementAttempts: 1 });
      const initial = await svc.decide(makeContext({
        trustLevel: 3 as TrustLevel,
        trustScore: 500,
      }));
      ...
      const second = await svc.decide(makeContext({
        trustLevel: 3 as TrustLevel,
        trustScore: 500,
  ...
  describe('retrieval', () => {
    it('retrieves decision by ID with correct tenant', async () => {
      const result = await service.decide(makeContext());
      const decision = await service.getDecision(result.decision.id, 'tenant-1');
      expect(decision).not.toBeNull();
    ...
    it('returns null for wrong tenant', async () => {
      const result = await service.decide(makeContext());
      const decision = await service.getDecision(result.decision.id, 'tenant-other');

...
describe("Merkle Aggregation Service", () => {
  ...
  it("should verify inclusion", async () => {
    ...
    expect(service.verifyInclusion(anchorId, "item-1", "wrong-data")).toBe(
      false,
    );
  });
...
describe("Pedersen Commitments", () => {
  it("should create and verify commitment", () => {
    ...
    expect(
      verifyCommitment(commitment.commitment, value, commitment.blinding),
    ).toBe(true);
    expect(
      verifyCommitment(commitment.commitment, value + 1, commitment.blinding),
    ).toBe(false);
  });
});
...
describe("ZK Proof Service", () => {
  ...
  it("should verify different proof types", () => {
    ...
    // Membership proof
    const membershipResult = generateMembershipProof("admin", [
      "admin",
      "user",
    ]);

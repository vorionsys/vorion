...
describe("Q3: Role Gates (Dual-Layer)", () => {
  describe("Kernel validation (fail-fast)", () => {
    ...
    it("should reject invalid combinations", () => {
      // R-L2 (Planner) requires at least T1
      expect(ROLE_GATE_MATRIX["R-L2"]["T0"]).toBe(false);
      // R-L4 (Architect) requires at least T3
      expect(ROLE_GATE_MATRIX["R-L4"]["T0"]).toBe(false);
      expect(ROLE_GATE_MATRIX["R-L4"]["T1"]).toBe(false);
      expect(ROLE_GATE_MATRIX["R-L4"]["T2"]).toBe(false);
      // R-L6 (Domain Authority) requires at least T5
      expect(ROLE_GATE_MATRIX["R-L6"]["T3"]).toBe(false);
    });
...
describe("Q4: Weight Presets (Hybrid Spec + Deltas)", () => {
  describe("Canonical presets", () => {
    it("should load canonical high_confidence preset", () => {
      ...
      expect(preset.capabilityWeight).toBe(0.25);
      expect(preset.behaviorWeight).toBe(0.3);
      expect(preset.contextWeight).toBe(0.15);
    });

    it("should validate preset weights sum to ~1.0", () => {
      for (const [name, preset] of Object.entries(CANONICAL_TRUST_PRESETS)) {
        const sum =
...
describe("Q5: Creation Modifiers (Instantiation Time)", () => {
  describe("Creation type validation", () => {
    ...
    it("should apply creation modifiers", () => {
      ...
      // Verify modifier application: baseline + modifier = final
      const baseline = 500;
      const clonedScore = baseline + CREATION_TYPE_MODIFIERS["cloned"];
      expect(clonedScore).toBe(450);
      const promotedScore = baseline + CREATION_TYPE_MODIFIERS["promoted"];
      expect(promotedScore).toBe(550);
    });

...
describe('Q2: Context Policy (Immutable at Instantiation)', () => {
  describe('Context immutability', () => {
    ...
    it('should restrict enterprise context to T0-T5 (max 900)', () => {
      // Enterprise ceiling is 900, which is within T6 range (876-950)
      const score = 900;
...
describe('Q3: Role Gates (Dual-Layer)', () => {
  describe('Kernel validation (fail-fast)', () => {
    ...
    it('should restrict R-L0 to T0 only', () => {
      ...
      expect(ROLE_GATE_MATRIX['R-L0']['T6']).toBe(false);
      expect(ROLE_GATE_MATRIX['R-L0']['T7']).toBe(false);
    });
    ...
    it('should restrict R-L1 to T0-T1', () => {
      ...
      expect(ROLE_GATE_MATRIX['R-L1']['T6']).toBe(false);
      expect(ROLE_GATE_MATRIX['R-L1']['T7']).toBe(false);
    });
    ...
    it('should restrict R-L2 to T0-T2', () => {
      ...
      expect(ROLE_GATE_MATRIX['R-L2']['T6']).toBe(false);
      expect(ROLE_GATE_MATRIX['R-L2']['T7']).toBe(false);
    });
    ...
    it('should restrict R-L3 to T0-T3', () => {
      ...
      expect(ROLE_GATE_MATRIX['R-L3']['T6']).toBe(false);
      expect(ROLE_GATE_MATRIX['R-L3']['T7']).toBe(false);
    });
    ...
    it('should restrict R-L4 to T0-T4', () => {
      ...
      expect(ROLE_GATE_MATRIX['R-L4']['T6']).toBe(false);
      expect(ROLE_GATE_MATRIX['R-L4']['T7']).toBe(false);
    });

    it('should allow R-L5 T0-T5, R-L6 T0-T6, R-L7/R-L8 all tiers (T0-T7)', () => {
      // R-L5: T0-T5 access
      const rl5Tiers: TrustTier[] = ['T0', 'T1', 'T2', 'T3', 'T4', 'T5'];
      for (const tier of rl5Tiers) {
        expect(ROLE_GATE_MATRIX['R-L5'][tier]).toBe(true);
      }
      expect(ROLE_GATE_MATRIX['R-L5']['T6']).toBe(false);
      expect(ROLE_GATE_MATRIX['R-L5']['T7']).toBe(false);

      // R-L6: T0-T6 access
      const rl6Tiers: TrustTier[] = ['T0', 'T1', 'T2', 'T3', 'T4', 'T5', 'T6'];
      for (const tier of rl6Tiers) {
        expect(ROLE_GATE_MATRIX['R-L6'][tier]).toBe(true);
      }
      expect(ROLE_GATE_MATRIX['R-L6']['T7']).toBe(false);

      // R-L7 and R-L8: Full access T0-T7
      const fullAccessRoles: RoleLevel[] = ['R-L7', 'R-L8'];
      const allTiers: TrustTier[] = ['T0', 'T1', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7'];
    ...
    it('should have all 8 tiers defined per role', () => {

...
function createTestRequest(overrides: Partial<GovernanceRequest> = {}): GovernanceRequest {
  return {
    requestId: 'req-001',
    entityId: 'agent-test',
    trustLevel: 1,
...
describe('GovernanceProofBridge', () => {
  ...
  describe('evaluateWithProof', () => {
    ...
    it('should handle multiple sequential evaluations', async () => {
      ...
      const r1 = await bridge.evaluateWithProof(createTestRequest({ requestId: 'req-1' }));
      const r2 = await bridge.evaluateWithProof(createTestRequest({ requestId: 'req-2' }));
      const r3 = await bridge.evaluateWithProof(createTestRequest({ requestId: 'req-3' }));

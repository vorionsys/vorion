...
export async function createServer(deps?: {
  ...
  server.get(
    "/ready",
    async (_request, reply): Promise<ReadinessCheckResponse> => {
      const checks: Record<string, HealthCheckResult> = {};
  ...
  server.register(
    async (api) => {
      ...
      api.post<{ Body: IntentSubmitBody | SDKIntentSubmitBody }>(
        ...
        async (
          request: FastifyRequest<{
            Body: IntentSubmitBody | SDKIntentSubmitBody;
          }>,
          reply: FastifyReply,
        ) => {
          ...
          if ("agentId" in body && "action" in body) {
            ...
            if (!agentId || !action?.type || !action?.resource) {
              return reply.status(400).send({
                error: {
                  ...
                  message:
                    "Missing required fields: agentId, action.type, action.resource",
                },
              });
            }
            ...
            apiLogger.info(
              { intentId, agentId, action: action.type, allowed, tier },
              "Intent processed",
            );
            ...
            // Legacy format
            const { entityId, goal, context, metadata } =
              body as IntentSubmitBody;
      ...
      api.get<{ Params: { id: ID } }>(
        "/intents/:id",
        async (
          request: FastifyRequest<{ Params: { id: ID } }>,
          reply: FastifyReply,
        ) => {
          const intent = await intentService.get(
            request.params.id,
            "__system__",
          );
      ...
      api.post<{ Body: SDKIntentCheckBody }>(
        "/intents/check",
        async (
          request: FastifyRequest<{ Body: SDKIntentCheckBody }>,
          reply: FastifyReply,
        ) => {
      ...
      api.post<{ Body: SDKAdmitBody }>(
        "/trust/admit",
        async (
          request: FastifyRequest<{ Body: SDKAdmitBody }>,
          reply: FastifyReply,
        ) => {
      ...
      api.post<{ Body: GovernanceRequest }>(
        "/governance/evaluate",
        async (
          request: FastifyRequest<{ Body: GovernanceRequest }>,
          reply: FastifyReply,
        ) => {
          const body = request.body;
          ...
          const { result, proofId } =
            await governanceBridge.evaluateWithProof(body);
      ...
      api.post<{ Body: GovernanceRule }>(
        "/governance/rules",
        async (
          request: FastifyRequest<{ Body: GovernanceRule }>,
          reply: FastifyReply,
        ) => {
        ...
        async (
          ...
          return reply.status(501).send({
            error: {
              ...
              message:
                "Boot camp must be run programmatically via PromotionService. " +
      ...
      api.post<{ Body: ConstraintValidateBody }>(
        ...
        async (
          request: FastifyRequest<{ Body: ConstraintValidateBody }>,
          reply: FastifyReply,
        ) => {
    ...
    (error: Error & { statusCode?: number; code?: string }, request, reply) => {
      ...
      reply.status(error.statusCode ?? 500).send({
        error: {
          ...
          message:
            config.env === "production" ? "An error occurred" : error.message,
        },
      });
    },
  );

  ...
  constructor(additionalSchemas?: ActionSchema[]) {
    super(
      createLayerConfig(3, "Schema Conformance", {
        description:
          "Validates payload action and fields against known schemas",
        tier: "input_validation",
  ...
  async execute(input: LayerInput): Promise<LayerExecutionResult> {
    ...
    if (action === undefined || action === null) {
      findings.push({
        ...
        description:
          'Payload has no "action" field — cannot determine request type',
        evidence: ["payload.action is undefined"],
        remediation:
          'Include an "action" field in the payload (e.g., "query", "execute", "read")',
      });
      ...
      case "string[]":
        if (
          ...
          return this.typeError(
            field,
            expectedType,
            Array.isArray(value) ? "mixed array" : typeof value,
          );
        }
        break;
      case "number[]":

...
export function computeEfficiencyMetric(
  ...
  const zeroMetric: TrustEfficiencyMetric = {
    successRatio: 0,
    failureRatio: 0,
    authorizedDomainAttempts: 0,
    cascadingFailures: 0,
    circuitBreakerTrips: 0,
    scheduleOptimality: 0,
  };
  ...
  // Aggregate success ratio from each event's metrics
  const totalSuccessRatio =
    agentEvents.reduce((sum, e) => sum + e.metrics.successRatio, 0) /
    agentEvents.length;

  // Aggregate authorization attempts (allowed = authorized domain attempts)
  const totalAuthorizedAttempts = agentEvents.reduce(
    (sum, e) => sum + e.metrics.authorizationHistory.allowed,
    0,
  );

  // Total cascading failures across all events
  const totalCascadingFailures = agentEvents.reduce(
    (sum, e) => sum + e.metrics.cascadingFailures,
    0,
  );

  // Circuit breaker trips: count events where ceiling was applied (score exceeded limit)
  const totalCircuitBreakerTrips = agentEvents.filter(
    (e) => e.ceilingApplied,
  ).length;

  // Schedule optimality derived from average execution efficiency
  const avgExecutionEfficiency =
    agentEvents.reduce((sum, e) => sum + e.metrics.executionEfficiency, 0) /
    agentEvents.length;

  // Compute efficiency: successRate * (1 - avgLatencyNormalized)
  // We use (1 - executionEfficiency) as a proxy for latency since higher
  // execution efficiency means lower normalised latency.
  const scheduleOptimality = totalSuccessRatio * avgExecutionEfficiency;

  return {
    successRatio: totalSuccessRatio,
    failureRatio: 1 - totalSuccessRatio,
    authorizedDomainAttempts: totalAuthorizedAttempts,
    cascadingFailures: totalCascadingFailures,

...
export class TrustEngine extends EventEmitter {
  ...
  private _decayCheckIntervalMs: number;
  ...
  async initializeEntity(entityId: ID, initialLevel: TrustLevel = 1): Promise<TrustRecord> {
    ...
    const initialFactorScores: Record<string, number> = {};
    for (const code of FACTOR_CODES) {
   ...
   */
  private calculateFactorScores(signals: TrustSignal[]): Record<string, number> {
    const factorSignals: Record<string, TrustSignal[]> = {};

...
export class TrustEngine {
   ...
   */
  async calculate(entityId: ID, options?: TrustOperationOptions): Promise<TrustCalculation> {
    ...
    try {
      ...
      if (options?.tenantId) {
        ...
        logger.warn(
          { entityId },
          'SECURITY WARNING: calculate() called without tenantId - this will be required in future versions'
        );
      }
   ...
   * @param options - Operation options including tenantId
   */
  async getScore(entityId: ID, options?: TrustOperationOptions): Promise<TrustRecord | undefined> {
    try {
      const db = await this.ensureInitialized();
      ...
      if (options?.tenantId) {
        ...
        logger.warn(
          { entityId },
          'SECURITY WARNING: getScore() called without tenantId - this will be required in future versions'
        );
      }
      ...
      const returnComponents: TrustComponents = {
   ...
   * @param options - Operation options including tenantId
   */
  async recordSignal(signal: TrustSignalType, options?: TrustOperationOptions): Promise<void> {
    try {
      const db = await this.ensureInitialized();
      ...
      if (options?.tenantId) {
        ...
        logger.warn(
          ...
          'SECURITY WARNING: recordSignal() called without tenantId - this will be required in future versions'
        );
      }
  ...
  async initializeEntity(
    ...
    try {
      ...
      if (options?.tenantId) {
        ...
        logger.warn(
          { entityId },
          'SECURITY WARNING: initializeEntity() called without tenantId - this will be required in future versions'
        );
      }
   ...
   */
  private static initialFactorScores(): Record<string, number> {
    const scores: Record<string, number> = {};
    for (const code of FACTOR_CODES) {
      scores[code] = 0.5;
    }
  ...
  async getACITrustContext(
    ...
    return {
      car: parsedCarId,
      identity: `${parsedCarId.registry}.${parsedCarId.organization}.${parsedCarId.agentClass}` as CARIdentity,
...
// ============================================================================
// CAR Integration Re-exports
// ============================================================================

// Re-export from @vorionsys/contracts/car
export type { CertificationTier, RuntimeTier, CapabilityLevel, ParsedCAR, ParsedACI, CARIdentity, ACIIdentity };

...
export class TOTPService {
  ...
  generateTOTPSecret(userId: string, email: string): TOTPSecret {
    ...
    // keyuri(accountName, issuer, secret) - returns otpauth:// URL
    const otpauthUrl = authenticator.keyuri(email, this.config.issuer, secret);
  ...
  async verifyToken(secret: string, token: string): Promise<boolean> {
    try {
      ...
      // Configure authenticator options before verification
      ...
      // check() returns boolean directly in otplib v12
      const isValid = authenticator.check(token, secret);

      logger.debug('Token verification completed', { valid: isValid });
      return isValid;
    } catch (error) {

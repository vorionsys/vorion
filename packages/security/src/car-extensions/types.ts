...
export interface AgentIdentity {
  ...
  did: string;
  /** Agent's CAR ID string (with optional extensions) */
  carId: string;
...
export const BehaviorMetricsSchema = z.object({
  ...
  maxLevelUsed: z.number().int().min(0).max(5),
...
export interface CARExtension {
  ...
  /** Semantic version */
  version: string;
  /** Short code for CAR ID strings (e.g., "gov", "audit") */
  shortcode: string;
  /** Publisher DID */
  publisher: string;
  /** Extension description */
  description: string;
  /** Required CAR ID core version (semver range) */

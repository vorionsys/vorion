...
export class CARExtensionService {
  ...
  /**
   * Load extensions for an agent based on their CAR ID string
   *
   * Parses the CAR ID string's extension suffix and returns the
   * corresponding registered extensions.
   *
   * @param carId - Agent's CAR ID string (may include #extension suffixes)
   * @returns Array of loaded extensions
   *
   * @example
   * ```typescript
   * // CAR ID: a3i.vorion.agent:FHC-L3@1.0.0#gov,audit
  ...
  loadAgentExtensions(carId: string): CARExtension[] {
    ...
    if (shortcodes.length === 0) {
      logger.debug({ carId }, 'No extensions declared in CAR ID string');
      return [];
    }
   ...
   *
   * @param carId - Agent's CAR ID string

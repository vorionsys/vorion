 ...
 *
 * PRODUCTION NOTE:
 * This module uses @noble/post-quantum (audited pure-JS FIPS 203 implementation)
 * when available, falling back to a hash-based reference implementation.
...
// =============================================================================
// @noble/post-quantum Integration
// =============================================================================

interface NobleKEMModule {
  keygen(seed?: Uint8Array): { publicKey: Uint8Array; secretKey: Uint8Array };
  encapsulate(publicKey: Uint8Array, seed?: Uint8Array): { cipherText: Uint8Array; sharedSecret: Uint8Array };
  decapsulate(cipherText: Uint8Array, secretKey: Uint8Array): Uint8Array;
}

let nobleModules: Record<string, NobleKEMModule> | null = null;
let nobleModuleChecked = false;

/**
 * Load @noble/post-quantum ML-KEM modules
 */
async function loadNobleModule(): Promise<Record<string, NobleKEMModule> | null> {
  if (nobleModuleChecked) {
    return nobleModules;
  }

  nobleModuleChecked = true;

  try {
    const mod = await import('@noble/post-quantum/ml-kem.js');
    nobleModules = {
      kyber512: mod.ml_kem512,
      kyber768: mod.ml_kem768,
      kyber1024: mod.ml_kem1024,
    };
    logger.info('@noble/post-quantum ML-KEM module loaded (FIPS 203)');
    return nobleModules;
  } catch {
    // Module not available
  }

  logger.warn('No ML-KEM module available, using reference implementation');
  return null;
}
...
export class KyberService {
  ...
  private nobleAvailable: boolean = false;
  ...
  /**
   * Initialize the service and detect @noble/post-quantum bindings
   */
  async initialize(): Promise<void> {
    if (this.config.preferNativeBindings) {
      const noble = await loadNobleModule();
      this.nobleAvailable = noble !== null;
    }

    logger.info(
      { nobleAvailable: this.nobleAvailable },
      'Kyber service initialization complete'
    );
  }

  /**
   * Check if @noble/post-quantum ML-KEM is available
   */
  isNativeAvailable(): boolean {
    return this.nobleAvailable;
  }
  ...
  async generateKeyPair(
    ...
    try {
      if (this.nobleAvailable && nobleModules) {
        const noble = nobleModules[parameterSet];
        if (noble) {
          const result = noble.keygen();
          return {
      ...
      logger.debug(
        {
          ...
          implementation: this.nobleAvailable ? 'noble' : 'reference',
        },
  ...
  async encapsulate(
    ...
    try {
      if (this.nobleAvailable && nobleModules) {
        const noble = nobleModules[parameterSet];
        if (noble) {
          const result = noble.encapsulate(publicKey);
          return {
  ...
  async decapsulate(
    ...
    try {
      if (this.nobleAvailable && nobleModules) {
        const noble = nobleModules[parameterSet];
        if (noble) {
          const sharedSecret = noble.decapsulate(ciphertext, privateKey);

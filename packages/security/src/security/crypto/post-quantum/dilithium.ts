 ...
 *
 * PRODUCTION NOTE:
 * This module uses @noble/post-quantum (audited pure-JS FIPS 204 implementation)
...
interface NobleDSAModule {
  keygen(seed?: Uint8Array): { publicKey: Uint8Array; secretKey: Uint8Array };
  sign(message: Uint8Array, secretKey: Uint8Array): Uint8Array;
  verify(signature: Uint8Array, message: Uint8Array, publicKey: Uint8Array): boolean;
}

let nobleModules: Record<string, NobleDSAModule> | null = null;
...
/**
 * Load @noble/post-quantum ML-DSA modules
 */
async function loadNobleModule(): Promise<Record<string, NobleDSAModule> | null> {
  ...
  try {
    ...
    nobleModules = {
      dilithium2: mod.ml_dsa44,
      dilithium3: mod.ml_dsa65,
      dilithium5: mod.ml_dsa87,
    };
    logger.info('@noble/post-quantum ML-DSA module loaded (FIPS 204)');
    return nobleModules;
  } catch {
    // Module not available
  }

  logger.warn('No ML-DSA module available, using reference implementation');
  return null;
}
...
export class DilithiumService {
  ...
  /**
   * Check if @noble/post-quantum ML-DSA is available
   */
  ...
  async sign(
    ...
    try {
      ...
      if (this.nobleAvailable && nobleModules) {
        const noble = nobleModules[parameterSet];
        if (noble) {
          signature = noble.sign(messageToSign, privateKey);
        } else {
  ...
  async verify(
    ...
    try {
      ...
      if (this.nobleAvailable && nobleModules) {
        const noble = nobleModules[parameterSet];
        if (noble) {
          valid = noble.verify(signature, messageToVerify, publicKey);
        } else {

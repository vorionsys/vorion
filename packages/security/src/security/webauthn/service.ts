...
export class WebAuthnService {
  ...
  async verifyRegistration(input: VerifyRegistrationInput): Promise<RegistrationResult> {
    ...
    // Check if credential already exists (v10: credentialID is directly on registrationInfo)
    const existingCredential = await this.store.getCredentialByCredentialId(
      registrationInfo.credentialID
    );
    if (existingCredential) {
      logger.warn(
        { userId: validated.userId, credentialId: registrationInfo.credentialID },
    ...
    // Create credential record (v10: properties are directly on registrationInfo)
    const credentialName = validated.credentialName ?? this.generateCredentialName();
    const credential: WebAuthnCredential = {
      id: secureRandomId(),
      credentialId: registrationInfo.credentialID,
      publicKey: this.uint8ArrayToBase64Url(registrationInfo.credentialPublicKey),
  ...
  async verifyAuthentication(input: VerifyAuthenticationInput): Promise<AuthenticationResult> {
    ...
    // Verify authentication response (v10: uses 'authenticator' instead of 'credential')
    ...
    try {
      verification = await verifyAuthenticationResponse({
        ...
        authenticator: {
          ...
          credentialPublicKey: this.base64UrlToUint8Array(credential.publicKey),

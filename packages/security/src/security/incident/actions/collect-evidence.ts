...
let evidenceService: EvidenceCollectionService | null = null;
...
export function getEvidenceCollectionService(): EvidenceCollectionService {
  if (!evidenceService) {
    throw new Error(
      'No evidence collection service configured. Call setEvidenceCollectionService() with a real implementation before use. ' +
      'For tests, use createMockEvidenceCollectionService().'
    );
  }
  return evidenceService;
}

/** Create a mock evidence collection service for testing only. */
export function createMockEvidenceCollectionService(): EvidenceCollectionService {
  return new MockEvidenceCollectionService();
}
...
async function executeEvidenceCollection(context: ActionContext): Promise<ActionResult> {
  const service = getEvidenceCollectionService();
  ...
  for (const target of targets) {
    ...
    try {
      ...
      switch (target.type) {
        case 'logs': {
          const logResult = await service.collectLogs(
            target.source,
          ...
          if (logResult.success) {
            result.hash = await service.calculateHash(logResult.location);
            await addEvidence({
              type: 'log',
        ...
        case 'system_state': {
          const stateResult = await service.captureSystemState(target.source);
          result.collected = stateResult.success;
          result.location = stateResult.location;
          result.size = stateResult.size;

          if (stateResult.success) {
            result.hash = await service.calculateHash(stateResult.location);
            await addEvidence({
              type: 'other',
        ...
        case 'network_capture': {
          const pcapResult = await service.captureNetworkTraffic(
            target.source,
          ...
          if (pcapResult.success) {
            result.hash = await service.calculateHash(pcapResult.location);
            await addEvidence({
        ...
        case 'memory_dump': {
          const memResult = await service.captureMemoryDump(target.source);
          result.collected = memResult.success;
          result.location = memResult.location;
          result.size = memResult.size;

          if (memResult.success) {
            result.hash = await service.calculateHash(memResult.location);
            await addEvidence({
              type: 'memory_dump',
        ...
        case 'configuration': {
          const configResult = await service.exportConfiguration(target.source);
          result.collected = configResult.success;
          result.location = configResult.location;
          result.size = configResult.size;

          if (configResult.success) {
            result.hash = await service.calculateHash(configResult.location);
            await addEvidence({
              type: 'configuration',
        ...
        case 'database_audit': {
          const auditResult = await service.exportDatabaseAudit(
            target.source,
            target.timeRange || timeRange
          );
          ...
          if (auditResult.success) {
            result.hash = await service.calculateHash(auditResult.location);
            await addEvidence({
              type: 'log',
        ...
        case 'file_snapshot': {
          const snapResult = await service.createFileSnapshot(target.source);
          result.collected = snapResult.success;
          result.location = snapResult.location;
          result.size = snapResult.size;

          if (snapResult.success) {
            result.hash = await service.calculateHash(snapResult.location);
            await addEvidence({
              type: 'file',

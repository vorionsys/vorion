...
import { TrustBand, ActionType } from '@vorionsys/contracts/v2';
...
export function trustLevelToTrustBand(level: TrustLevel): TrustBand {
  // Map legacy 6-tier system to new 8-tier system
  switch (level) {
    case 0:
      return TrustBand.T0_SANDBOX;      // Was T0_UNTRUSTED
    case 1:
      return TrustBand.T1_OBSERVED;     // Was T1_SUPERVISED
    case 2:
      return TrustBand.T2_PROVISIONAL;  // Was T2_CONSTRAINED
    case 3:
      return TrustBand.T4_STANDARD;     // Was T3_TRUSTED (mapped to T4)
    case 4:
      return TrustBand.T6_CERTIFIED;    // Was T4_AUTONOMOUS (mapped to T6)
    case 5:
      return TrustBand.T7_AUTONOMOUS;   // Was T5_MISSION_CRITICAL (mapped to T7)
    default:
...
export function trustScoreToTrustBand(score: TrustScore): TrustBand {
  // 8-tier trust bands per ATSF spec

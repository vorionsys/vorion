...
export class ContextValidator {
  ...
  validateContent(
    ...
    if (contentIntegrity.signatureRequired) {
      ...
      // Verify HMAC-SHA256 content signature
  ...
  /**
   * Verify content signature using HMAC-SHA256.
   *
   * Expects signature in format `hmac-sha256:<base64_signature>`.
   * The HMAC key is derived from the VORION_CONTENT_HMAC_KEY environment
   * variable. Falls back to length check if no key is configured.
   *
  ...
  private verifyContentSignature(content: unknown, signature: string): boolean {
    const hmacKey = process.env['VORION_CONTENT_HMAC_KEY'];
    if (!hmacKey) {
      logger.warn('VORION_CONTENT_HMAC_KEY not set; signature verification requires HMAC key');
      return false;
    }

    // Parse signature format: hmac-sha256:<base64>
    ...
    const expectedSig = createHmac('sha256', hmacKey).update(canonical).digest('base64');

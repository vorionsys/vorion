...
export class InstructionValidator {
  ...
  verifySignedSource(instruction: string, signature: string, source: string): boolean {
    ...
    // Verify signature using HMAC-SHA256 with a shared key.
    // Signature format: hmac-sha256:<base64_signature>
    try {
      const hmacKey = process.env['VORION_INSTRUCTION_HMAC_KEY'];
      if (!hmacKey) {
        logger.warn('VORION_INSTRUCTION_HMAC_KEY not set; cannot verify instruction signature');
        return false;
      }

      const parts = signature.split(':');
      if (parts.length !== 2 || parts[0] !== 'hmac-sha256') {
        logger.warn({ format: parts[0] }, 'Unsupported instruction signature format');
        return false;
      }

      const providedSig = Buffer.from(parts[1], 'base64');
      const expectedSig = createHmac('sha256', hmacKey).update(instruction).digest();

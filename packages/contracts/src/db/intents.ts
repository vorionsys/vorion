...
export const intentProcessingLog = pgTable(
  ...
  (table) => ({
    intentIdIdx: index("intent_processing_log_intent_id_idx").on(
      table.intentId,
    ),
    tenantIdIdx: index("intent_processing_log_tenant_id_idx").on(
      table.tenantId,
    ),
    timestampIdx: index("intent_processing_log_timestamp_idx").on(
      table.timestamp,
    ),
  }),
);

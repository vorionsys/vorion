...
describe("validateCAR", () => {
  ...
  it("auto-migrates legacy format with warning", () => {
    ...
    expect(
      result.warnings.some(
        (w) =>
          w.code === "LEGACY_FORMAT" || w.code === "LEGACY_FORMAT_MIGRATED",
      ),
    ).toBe(true);
    expect(result.parsed).toBeDefined();
  });
  ...
  it("warns about security domain", () => {
    ...
    expect(result.valid).toBe(true);
    expect(result.warnings.some((w) => w.code === "SECURITY_DOMAIN")).toBe(
      true,
    );
  });
  ...
  it("warns about L7 autonomous level", () => {
    ...
    expect(result.valid).toBe(true);
    expect(result.warnings.some((w) => w.code === "L7_AUTONOMOUS_LEVEL")).toBe(
      true,
    );
  });
});

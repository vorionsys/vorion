...
import { BASIS_CANONICAL_PRESETS, T3_BASELINE } from '../../../src/trust/presets.js';
...
describe('BMAD Presets', () => {
  describe('BMAD_DELTAS', () => {
    ...
    it('orchestrator override should boost CT-OBS and OP-HUMAN', () => {
      ...
      expect(orch['CT-OBS']).toBeGreaterThan(0.10);
  ...
  describe('createBmadPreset', () => {
    ...
    it('should merge BMAD delta with canonical preset', () => {
      ...
      expect(result['CT-OBS']).toBe(BMAD_DELTAS['orchestrator_override']!['CT-OBS']);
  ...
  describe('bmadAgentTrustConfigs', () => {
    it('should contain all BMAD agent configs', () => {
      expect(Object.keys(bmadAgentTrustConfigs).length).toBeGreaterThanOrEqual(15);
    });

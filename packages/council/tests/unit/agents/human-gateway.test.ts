...
describe('TriageAgent', () => {
  ...
  it('should escalate on budget exceeded', () => {
    const state = createBaseState({
      metadata: { priority: 'medium', maxCost: 0.10 },
      plan: {
        steps: [],
        estimatedCost: 5.0,
        estimatedTime: 300,
...
describe('ContextBuilderAgent', () => {
  ...
  it('should build context summary with all sections', () => {
    const state = createBaseState({
      plan: {
        steps: [
          { id: 's1', description: 'Step 1', assignTo: 'advisor', estimatedCost: 0.05, estimatedTime: 60, dependencies: [], status: 'pending' },
        ],
        estimatedCost: 0.05,
        estimatedTime: 60,
        complexity: 'simple',
    ...
    const decision: EscalationDecision = {
      ...
      flags: [
        { type: 'BUDGET_EXCEEDED', severity: 'ERROR', title: 'Budget Exceeded', description: 'Over budget', detectedBy: 'triage', context: {} },
      ],
    };

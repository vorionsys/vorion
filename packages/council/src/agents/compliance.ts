...
Please analyze for compliance and ethics issues.
      ...
      return {
        ...
        compliance: {
          ...
          containsPII:
            result.containsPII || (state.compliance?.containsPII ?? false),
      ...
      return {
        ...
        errors: [
          ...state.errors,
          {
            ...
            message:
              error instanceof Error ? error.message : "Compliance check error",
  ...
  private parseComplianceResult(content: string): {
    ...
    try {
      ...
      console.warn(
        `[${this.agentId}] Could not parse compliance result, assuming passed`,
      );
      return {
        passed: true,
        issues: [],
  ...
  private getHighestSensitivity(
    ...
    return hierarchy[Math.max(index1, index2)] as
      | "public"
      | "internal"
      | "confidential"
      | "restricted";
...
export async function runComplianceCheck(
  ...
  const results = await Promise.all(agents.map((agent) => agent.check(state)));
  ...
  const finalState = results.reduce((combined, result) => {
    return {
      ...
      compliance: {
        passed:
          (combined.compliance?.passed ?? true) &&
        ...
        containsPII:
          (combined.compliance?.containsPII ?? false) ||
          (result.compliance?.containsPII ?? false),
        sensitivityLevel:
          combined.compliance?.sensitivityLevel === "restricted" ||
          result.compliance?.sensitivityLevel === "restricted"
            ? "restricted"
            : combined.compliance?.sensitivityLevel === "confidential" ||
                result.compliance?.sensitivityLevel === "confidential"
              ? "confidential"
              : combined.compliance?.sensitivityLevel === "internal" ||
                  result.compliance?.sensitivityLevel === "internal"
                ? "internal"
                : "public",
        checkedBy: [
          ...new Set([
            ...(combined.compliance?.checkedBy || []),
            ...(result.compliance?.checkedBy || []),
          ]),
        ],
      },
  ...
  console.log(
    `[COMPLIANCE TEAM] Check complete. Passed: ${finalState.compliance?.passed}, Issues: ${finalState.compliance?.issues.length}`,
  );

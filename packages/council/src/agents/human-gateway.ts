...
export class TriageAgent {
  ...
  analyze(state: CouncilState): EscalationDecision {
    console.log(
      `[${this.name.toUpperCase()}] Analyzing request for escalation triggers...`,
    );
    ...
    if (
      state.plan &&
      state.plan.estimatedCost > (state.metadata.maxCost || Infinity)
    ) {
      ...
      flags.push({
        ...
        detectedBy: this.name,
        context: {
          estimated: state.plan.estimatedCost,
          max: state.metadata.maxCost,
        },
      });
    }
    ...
    if (state.output && state.output.confidence < 0.7) {
      ...
      escalationType =
        escalationType === "UNKNOWN" ? "LOW_CONFIDENCE" : escalationType;
    ...
    if (state.qa && state.qa.requiresRevision && state.qa.revisedCount > 2) {
      severity =
        severity === "CRITICAL" || severity === "HIGH" ? severity : "MEDIUM";
      escalationType =
        escalationType === "UNKNOWN" ? "QUALITY_FAILURE" : escalationType;
      ...
      flags.push({
        ...
        detectedBy: this.name,
        context: {
          revisedCount: state.qa.revisedCount,
          feedback: state.qa.feedback,
        },
      });
    }
    ...
    const required =
      flags.length > 0 || state.metadata.requiresHumanApproval || false;

    console.log(
      `[${this.name.toUpperCase()}] Escalation ${required ? "REQUIRED" : "NOT REQUIRED"}`,
    );
    console.log(
      `[${this.name.toUpperCase()}] Severity: ${severity}, Type: ${escalationType}, Flags: ${flags.length}`,
    );
...
export class ContextBuilderAgent {
  ...
  buildContext(
    ...
    console.log(
      `[${this.name.toUpperCase()}] Building context for reviewer...`,
    );
  ...
  private buildRecommendation(
    ...
    if (decision.escalationType === "COMPLIANCE_FAILURE") {
      ...
      parts.push(
        "- If PII is necessary, ensure it routes to self-hosted models",
      );
  ...
  private buildImpactAnalysis(
    ...
    if (state.plan) {
      parts.push(
        `**Time Impact:** ${state.plan.estimatedTime}s (~${Math.ceil(state.plan.estimatedTime / 60)} minutes)`,
      );
    }
    ...
    if (state.compliance && !state.compliance.passed) {
      parts.push(
        `**Compliance Risk:** HIGH - ${state.compliance.issues?.length || 0} violations detected`,
      );
    } else {
...
export class DecisionTrackerAgent {
  ...
  async createReview(
    ...
    console.log(
      `[${this.name.toUpperCase()}] Type: ${decision.escalationType}`,
    );
    console.log(
      `[${this.name.toUpperCase()}] Assigned to: ${decision.assignedTo || "Unassigned"}`,
    );
    console.log(
      `[${this.name.toUpperCase()}] Deadline: ${decision.deadline?.toISOString()}`,
    );
  ...
  async trackDecision(
    ...
    console.log(
      `[${this.name.toUpperCase()}] Reason: ${reason || "No reason provided"}`,
    );
...
export class HumanGatewayOrchestrator {
  ...
  async checkEscalation(state: CouncilState): Promise<CouncilState> {
    ...
    const reviewId = await this.trackerAgent.createReview(
      state,
      decision,
      context,
    );

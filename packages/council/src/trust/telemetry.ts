...
export const EVENT_FACTOR_MAP: Record<
  TelemetryEventType,
  { factorCode: string; baseDelta: number }
> = {
  // Core Trust (CT-)
...
export class TelemetryCollector {
  ...
  initAgent(
    agentId: string,
    agentName: string,
    initialTier: TierName = "T0",
  ): AgentTrustState {
    const tierDef =
      TRUST_TIERS.find((t) => t.name === initialTier) || TRUST_TIERS[0]!;
  ...
  private processEvent(event: TelemetryEvent): void {
    ...
    if (factorState) {
      ...
      factorState.score = Math.max(
        0,
        Math.min(1000, factorState.score + delta),
      );

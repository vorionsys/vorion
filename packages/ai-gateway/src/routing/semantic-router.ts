...
export class SemanticRouter {
  ...
  private initializeRoutes(): void {
    this.routes = [
      ...
      // Quick Q&A
      {
        ...
        modelName: "gemini-2.5-flash",
        temperature: 0.3,
        examples: ["what is", "how do I", "when should", "define"],
        historicalPerformance: {
          successRate: 91,
          avgLatency: 1200,
          avgCost: 0.005,
  ...
  async route(
    ...
    console.log(
      `[SEMANTIC_ROUTER] Routing query: ${query.substring(0, 50)}...`,
    );
  ...
  private calculateSimilarity(query: string, route: SemanticRoute): number {
    ...
    for (const example of route.examples) {
      ...
      const matches = exampleWords.filter((word) =>
  ...
  private generateReasoning(
    ...
    if (route.historicalPerformance.successRate >= 90) {
      parts.push(
        `with excellent historical success rate (${route.historicalPerformance.successRate}%)`,
      );
    }
  ...
  async reflect(
    ...
    const expectedSuccess =
      decision.route.historicalPerformance.successRate >= 80;
    ...
    if (actualSuccess !== expectedSuccess) {
      if (!actualSuccess) {
        ...
        learnings.push(
          `May need to adjust route pattern or switch to alternative model`,
        );
        ...
        learnings.push(
          `Route exceeded expectations: succeeded despite low confidence`,
        );
    ...
    if (latencyDiff > 0.5) {
      learnings.push(
        `Significant latency variance: ${latency}ms vs expected ${expectedLatency}ms`,
      );
    ...
    if (userFeedback === "negative") {
      learnings.push(
        "User provided negative feedback - route may not be optimal for this query type",
      );
      ...
      learnings.push(
        "User provided positive feedback - route is well-suited for this query type",
      );
  ...
  async getStatistics(): Promise<{
    ...
    const topRoutes = this.routes
      .sort(
        (a, b) =>
          b.historicalPerformance.totalRequests -
          a.historicalPerformance.totalRequests,
      )
      .slice(0, 5)
      .map((r) => ({
        route: r.pattern,
   ...
   */
  addRoute(
    route: Omit<SemanticRoute, "id" | "historicalPerformance">,
  ): SemanticRoute {

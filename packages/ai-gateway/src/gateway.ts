...
import { degradedTracker } from "./metrics/degraded-tracker.js";
...
function detectTaskType(
  text: string,
): "coding" | "reasoning" | "advisor" | "general" {
  const lowerText = text.toLowerCase();
...
export class AIGateway {
  ...
  constructor(config: { baseURL: string; apiKey: string }) {
    if (!config.apiKey) {
      throw new Error(
        "AIGateway requires an API key. Set LITELLM_MASTER_KEY or pass apiKey in config.",
      );
    }
    ...
    switch (taskType) {
      case "coding":
        return {
          ...
          route: "specialized",
          provider:
            request.metadata?.priority === "high" ? "anthropic" : "ollama",
      ...
      case "reasoning":
        return {
          ...
          route: "specialized",
          provider:
            request.metadata?.priority === "high" ? "anthropic" : "google",
  ...
  private async checkGreenRoute(
    ...
    const estimatedTokens =
      request.metadata?.estimatedTokens || this.estimateTokens(request);
    ...
    return {
      model: litellmModel,
      route: "green",
      provider: greenDecision.recommendedProvider as
        | "anthropic"
        | "google"
        | "ollama",
  ...
  async route(request: GatewayRequest): Promise<RoutingDecision> {
    ...
    if (privacyRoute) {
      console.log(
        `[GATEWAY] Route: PRIVACY -> ${privacyRoute.model} (${privacyRoute.reason})`,
      );
      return privacyRoute;
    }
    ...
    if (taskRoute) {
      console.log(
        `[GATEWAY] Route: SPECIALIZED -> ${taskRoute.model} (${taskRoute.reason})`,
      );
      return taskRoute;
    }
    ...
    if (greenRoute) {
      console.log(
        `[GATEWAY] Route: GREEN -> ${greenRoute.model} (${greenRoute.reason})`,
      );
      return greenRoute;
    }
    ...
    console.log(
      `[GATEWAY] Route: COST-OPTIMIZED -> ${costRoute.model} (${costRoute.reason})`,
    );
    return costRoute;
  }
  ...
  async chat(request: GatewayRequest): Promise<GatewayResponse> {
    ...
    let semanticDecision: Awaited<
      ReturnType<typeof semanticRouter.route>
    > | null = null;
    if (request.metadata?.taskType) {
      const userQuery =
        request.messages[request.messages.length - 1]?.content || "";
      semanticDecision = await semanticRouter.route(
        userQuery,
        request.metadata,
      );
    ...
    try {
      ...
      if (semanticDecision && semanticDecision.shouldReflect) {
        ...
        setImmediate(async () => {
          ...
          await semanticRouter
            .reflect(taskId, success, endTime - startTime)
            .catch((err) => {
      ...
      // Track successful (non-degraded) request
      degradedTracker.recordNormal();
      ...
      try {
        ...
        // Track degraded/fallback request
        degradedTracker.recordDegraded(fallbackModel, reason);
  ...
  private async trackCarbon(
    ...
    const actualModel = this.mapLiteLLMToActualModel(
      decision.model,
      decision.provider,
    );
   ...
   */
  private mapLiteLLMToActualModel(
    litellmModel: string,
    provider: string,
  ): string {
  ...
  private calculateCost(
    ...
    const pricing: Record<string, { input: number; output: number }> = {
      "general/premium": { input: 3.0, output: 15.0 },
      "general/balanced": { input: 0.15, output: 0.6 },
      "general/fast": { input: 0, output: 0 },
      "coding/expert": { input: 3.0, output: 15.0 },
      "coding/fast": { input: 0, output: 0 },
      "reasoning/complex": { input: 15.0, output: 75.0 },
      "reasoning/fast": { input: 0.15, output: 0.6 },
      "advisor/council": { input: 15.0, output: 75.0 },
      "advisor/consultation": { input: 3.0, output: 15.0 },

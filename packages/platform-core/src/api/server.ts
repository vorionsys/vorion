...
export async function createServer(): Promise<FastifyInstance> {
  ...
  const getVersionedPrefix = (v: string) => `/api/${v}`;
    ...
    async (v1Api) => {
      ...
      v1Api.addHook('preHandler', async (request, reply) => {
        ...
        try {
          ...
          const jtiValidation = validateJti(payload, config as unknown as { env: string; jwt: { requireJti: boolean } });
    ...
    async (api: FastifyInstance) => {
      ...
      api.addHook('preHandler', async (request, reply) => {
        ...
        try {
          ...
          // Validate jti claim
          const jtiValidation = validateJti(payload, config as any);
          if (!jtiValidation.valid) {
            tokenRevocationChecks.inc({ result: 'missing_jti' });
            return reply.status(401).send({
              error: { code: 'TOKEN_INVALID', message: jtiValidation.error },
      ...
      api.get('/trust/:entityId', async (request, reply) => {
        ...
        const trustRecord: TrustRecord | undefined = await trustEngine.getScore(params.entityId, ctx);
        if (!trustRecord) {
          return reply.status(404).send({
            success: false,
      ...
      api.post('/audit/verify', async (request, reply) => {
        ...
        const result: ChainIntegrityResult = await auditService.verifyChainIntegrity(tenantIdStr, {
      ...
      api.post('/policies', async (request: FastifyRequest, reply: FastifyReply) => {
        ...
        try {
          ...
          apiLogger.info(
            { policyId: policy.id, name: policy.name, tenantId: tenantIdStr },
            'Policy created'
          );
      ...
      api.get('/policies', async (request: FastifyRequest, reply: FastifyReply) => {
        ...
        if (query.status) listFilters.status = query.status as PolicyStatus;
      ...
      api.put('/policies/:id', async (request: FastifyRequest, reply: FastifyReply) => {
        ...
        try {
          ...
          apiLogger.info(
            { policyId: policy.id, version: policy.version, tenantId: tenantIdStr },
            'Policy updated'
          );
      ...
      api.delete('/policies/:id', async (request: FastifyRequest, reply: FastifyReply) => {
        ...
        apiLogger.info(
          { policyId: params.id, tenantId: tenantIdStr },
          'Policy deleted'
        );
      ...
      api.post('/webhooks', async (request, reply) => {
        ...
        try {
          ...
          apiLogger.info(
            { webhookId, tenantId: tenantIdStr, url: body.url },
            'Webhook registered'
          );
      ...
      api.delete('/webhooks/:id', async (request, reply) => {
        ...
        apiLogger.info(
          { webhookId: params.id, tenantId: tenantIdStr },
      ...
      api.post('/intent/gdpr/export', async (request, reply) => {
        ...
        const gdprAuthContext: GdprAuthorizationContext = {
          requestingUserId: user.sub ?? 'unknown',
          requestingUserTenantId: tenantIdStr,
          roles: (user.roles ?? []) as GdprRole[],
        ...
        apiLogger.info(
          { requestId: exportRequest.id, userId: body.userId, tenantId: tenantIdStr },
      ...
      api.delete('/intent/gdpr/data', async (request, reply) => {
        ...
        apiLogger.info(
          { userId: body.userId, tenantId: tenantIdStr, erasedBy: user.sub, counts: result.counts },

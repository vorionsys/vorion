 ...
 * Uses the Vorion Expression Engine for evaluating condition expressions.
 */
...
import {
  ...
  ParserError,
  EvaluatorError,
} from '../common/expression/index.js';
import type {
  Rule,
  RuleNamespace,
  EvaluationContext,
...
import { evaluateExpression, ExpressionError } from './expression-evaluator.js';

const logger = createLogger({ component: 'basis-evaluator' });

/** Global expression engine instance */
...
export class RuleEvaluator {
  ...
  private async evaluateRule(
    ...
    // Evaluate each evaluation step using the Expression Engine
    for (const evaluation of rule.evaluate) {
      try {
        const conditionMet = this.evaluateExpressionSafe(evaluation.condition, context);
        ...
        // Log the error but continue to next evaluation
        ...
        // On evaluation error, treat as condition not met and continue
      }
    }
  ...
  /**
   * Safely evaluate a condition expression string
   *
   * Uses the secure expression evaluator that supports:
   * - Logical operators: AND, OR, NOT (or &&, ||, !)
   * - Comparison operators: ==, !=, <, >, <=, >=
   * - String operations: contains(), startsWith(), endsWith(), matches()
   * - Arithmetic: +, -, *, /, %
   * - Variable interpolation from context (e.g., entity.trustLevel, intent.goal)
   * - Nested expressions with parentheses
   *
   * @param expression - The expression string to evaluate
   * @param context - The evaluation context
   * @returns boolean result of the expression
   * @throws ExpressionError on invalid expressions
   */

...
export const SENSITIVE_PATTERNS = {
  ...
  IPV6: [
    ...
    // Match IPv6 with :: prefix at word boundary or whitespace
    /(?:^|[\s,;])::(?:[0-9a-fA-F]{1,4}:){0,5}[0-9a-fA-F]{1,4}\b/g,
    /\b[0-9a-fA-F]{1,4}::(?:[0-9a-fA-F]{1,4}:){0,5}[0-9a-fA-F]{1,4}\b/g,
    // Match IPv6 loopback ::1
    /(?:^|[\s,;])::1(?:\s|$|[,;])/g,
  ],
...
export class ErrorSanitizer {
  ...
  sanitizeMessage(message: string): string {
    ...
    // If message becomes too short or just redaction markers, return appropriate message
    const cleanedMessage = sanitized.replace(/\[REDACTED\]/g, '').trim();
    if (cleanedMessage.length < 5) {
      // If we performed redactions, indicate sensitive data was removed

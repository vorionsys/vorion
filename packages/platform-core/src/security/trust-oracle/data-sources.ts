 ...
 *
 * Security Note: All external HTTP requests are made through the secureFetch wrapper
 * which provides SSRF protection including:
 ...
 * - Audit logging of all external requests
 */
...
} from '../../common/secure-fetch.js';
import { createLogger } from '../../common/logger.js';

const dataSourceLogger = createLogger({ component: 'trust-oracle-data-sources' });
...
/**
 * Default timeout for data source API requests (15 seconds).
 * Individual sources can override this via their config.
 */
...
export abstract class DataSource {
  ...
  /**
   * Make a secure HTTP request to the data source API.
   *
   * Uses secureFetch for SSRF protection:
   * - Validates URL scheme (HTTPS required in production)
   * - Blocks private IP ranges and internal hostnames
   * - Performs DNS resolution to detect rebinding attacks
   * - Enforces request timeout
   * - Logs all external requests for audit
   *
   * @param endpoint - API endpoint path (appended to apiEndpoint)
   * @param options - Request options (method, headers, body)
   * @returns Parsed JSON response
   * @throws DataSourceError if request fails
   */
  protected async makeRequest<T>(
    ...
    // Pre-validate the URL before making the request
    ...
    if (!urlValidation.valid) {
      dataSourceLogger.error(
        ...
        'Data source URL validation failed - potential SSRF attempt'
      );
      this.errorCount++;
      this.healthy = false;
    ...
    try {
      ...
      const response = await secureFetch(url, {
        ...
        // Allow the default domains for security services
      ...
      // Handle SecureFetchError specifically
      if (error instanceof SecureFetchError) {
        dataSourceLogger.warn(
          {
            source: this.name,
            url,
          ...
          'Secure fetch error in data source request'
        );
        throw new DataSourceError(
          this.name,
          `Secure fetch failed: ${error.message}`,
        );
      }

      // Re-throw DataSourceError as-is

...
class VendorMonitor {
  ...
  private async fetchDNSRecords(domain: string): Promise<DNSRecord[]> {
    // Simplified DNS lookup implementation
    ...
    try {
      ...
      records.push({ type: 'A', value: '', ttl: 3600 });
      records.push({ type: 'NS', value: '', ttl: 86400 });
      records.push({ type: 'MX', value: '', ttl: 3600 });
    } catch (error) {
  ...
  private async fetchDomainInfo(domain: string): Promise<DomainInfo | null> {
    ...
    // In production, use a proper WHOIS library or API
    return {
      domain,
      registrar: 'Unknown',

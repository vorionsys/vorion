...
let monitoringService: MonitoringScalingService = new MockMonitoringScalingService();
...
async function executeScaleMonitoring(context: ActionContext): Promise<ActionResult> {
  ...
  try {
    ...
    for (const serviceId of affectedServices) {
      try {
        const result = await monitoringService.increaseLogVerbosity(serviceId, 'debug');
        enhancements.push({
          type: 'log_verbosity',
          target: serviceId,
    ...
    const alertRules = await monitoringService.getAvailableAlertRules(incident.type);
    if (alertRules.length > 0) {
      try {
        const result = await monitoringService.enableAlertRules(alertRules);
        enhancements.push({
          type: 'alert_rule',
          target: 'alert-rules',
    ...
    const detectionRules = await monitoringService.getAvailableDetectionRules(incident.type);
    if (detectionRules.length > 0) {
      try {
        const result = await monitoringService.deployDetectionRules(detectionRules);
        enhancements.push({
          type: 'detection_rule',
    ...
    if (affectedServices.length > 0) {
      try {
        ...
        const result = await monitoringService.enableEnhancedMetrics(
          affectedServices,
          metricsToCollect
        );
        enhancements.push({
    ...
    if (incident.severity === 'P1' || incident.severity === 'P2') {
      for (const serviceId of affectedServices.slice(0, 3)) {
        // Limit to 3 services
        try {
          const result = await monitoringService.increaseTraceSampling(serviceId, 1.0); // 100% sampling
          enhancements.push({
            type: 'trace_sampling',
            target: serviceId,
    ...
    if (['data_breach', 'unauthorized_access', 'insider_threat'].includes(incident.type)) {
      try {
        ...
        const result = await monitoringService.deployHoneypots(honeypotConfig);
        enhancements.push({
          type: 'honeypot',
          target: 'honeypots',
          previousState: [],
  ...
  for (const enhancement of data.enhancements) {
    try {
      ...
      switch (enhancement.type) {
        case 'log_verbosity':
          restored = await monitoringService.restoreLogVerbosity(
            enhancement.target,
        ...
        case 'alert_rule':
          ...
          restored = await monitoringService.disableAlertRules(ruleIds);
          break;

        case 'detection_rule':
          restored = await monitoringService.removeDetectionRules(
            enhancement.newState as string[]
          );
          break;

        case 'metric_collection':
          restored = await monitoringService.disableEnhancedMetrics(
            enhancement.newState as string[]
          );
          break;

        case 'trace_sampling':
          restored = await monitoringService.restoreTraceSampling(
            enhancement.target,
        ...
        case 'honeypot':
          restored = await monitoringService.removeHoneypots(enhancement.newState as string[]);
          break;
      }

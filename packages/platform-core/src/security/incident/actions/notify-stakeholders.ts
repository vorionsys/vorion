...
let notificationService: StakeholderNotificationService = new MockStakeholderNotificationService();
...
async function executeNotification(context: ActionContext): Promise<ActionResult> {
  ...
  try {
    ...
    const stakeholders = await notificationService.getStakeholders(
      incident.severity,
      incident.type
    );
...
async function sendNotification(
  ...
  switch (channel) {
    case 'slack':
      return notificationService.sendSlack(address, template);

    case 'email':
      return notificationService.sendEmail(address, template);

    case 'pagerduty':
      const pdResult = await notificationService.sendPagerDuty(address, template);
    ...
    case 'sms':
      ...
      return notificationService.sendSms(address, smsMessage);

    case 'teams':
      return notificationService.sendTeams(address, template);

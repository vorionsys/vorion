...
let ipBlockingService: IpBlockingService = new MockIpBlockingService();
...
async function executeIpBlocking(context: ActionContext): Promise<ActionResult> {
  ...
  for (const target of ipTargets) {
    try {
      ...
      const existingBlock = await ipBlockingService.isBlocked(target.ipAddress);
      ...
      blockPromises.push(
        (async () => {
          try {
            const result = await ipBlockingService.blockAtFirewall(target.ipAddress, blockRule);
            results.push({
              ipAddress: target.ipAddress,
      ...
      blockPromises.push(
        (async () => {
          try {
            const result = await ipBlockingService.blockAtWaf(target.ipAddress, blockRule);
            results.push({
              ipAddress: target.ipAddress,
              blocked: result.success,
              blockType: isPermanent ? 'permanent' : 'temporary',
      ...
      blockPromises.push(
        (async () => {
          try {
            const result = await ipBlockingService.blockAtCdn(target.ipAddress, blockRule);
            results.push({
              ipAddress: target.ipAddress,
              blocked: result.success,
              blockType: isPermanent ? 'permanent' : 'temporary',
    ...
    if (target.ipRange) {
      try {
        const rangeResult = await ipBlockingService.blockIpRange(target.ipRange, {
...
async function rollbackIpBlocking(
  ...
  for (const block of data.blocks) {
    ...
    try {
      const success = await ipBlockingService.unblockIp(
        block.ipAddress,
        block.ruleId,
        block.service
      );

...
let credentialService: CredentialService = new MockCredentialService();
...
async function executeRevocation(context: ActionContext): Promise<ActionResult> {
  ...
  for (const userId of affectedUsers) {
    try {
      ...
      const activeCredentials = await credentialService.getActiveCredentials(userId);

      // 1. Terminate all sessions first (immediate effect)
      try {
        const sessionResult = await credentialService.terminateSessions(userId);
        results.push({
      ...
      try {
        const passwordResult = await credentialService.invalidatePassword(userId);
        results.push({
      ...
      if (activeCredentials.apiKeys.length > 0) {
        try {
          const apiKeyResult = await credentialService.revokeApiKeys(userId);
          results.push({
      ...
      if (activeCredentials.oauthClients.length > 0) {
        try {
          const oauthResult = await credentialService.revokeOAuthTokens(userId);
          results.push({
      ...
      if (activeCredentials.sshKeys.length > 0) {
        try {
          const sshResult = await credentialService.revokeSshKeys(userId);
          results.push({
      ...
      if (activeCredentials.certificates.length > 0) {
        try {
          const certResult = await credentialService.revokeCertificates(userId);
          results.push({

...
let isolationService: IsolationService = new MockIsolationService();
...
async function isolateTarget(
  ...
  switch (target.type) {
    case 'server': {
      const serverResult = await isolationService.isolateServer(target.identifier);
    ...
    case 'container': {
      const containerResult = await isolationService.isolateContainer(target.identifier);
    ...
    case 'service': {
      ...
      const serviceResult = await isolationService.isolateContainer(target.identifier);
    ...
    case 'network_segment': {
      const networkResult = await isolationService.isolateNetworkSegment(target.identifier);
    ...
    case 'user_account': {
      const userResult = await isolationService.disableUserAccount(target.identifier);
...
async function restoreTarget(
  ...
  switch (type) {
    case 'server':
      return isolationService.restoreServer(
        identifier,
        isolation.previousState as Record<string, unknown>
      );
    case 'container':
    case 'service':
      return isolationService.restoreContainer(
        identifier,
        isolation.previousState['network'] as string
      );
    case 'network_segment':
      return isolationService.restoreNetworkSegment(
        identifier,
        isolation.previousState['rules'] as unknown[]
      );
    case 'user_account':
      return isolationService.enableUserAccount(identifier);
    default:

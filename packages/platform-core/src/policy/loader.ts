...
import { createSystemTenantContext } from '../common/tenant-context.js';
...
export class PolicyLoader {
  ...
  async getPolicies(tenantId: ID, namespace?: string): Promise<Policy[]> {
    ...
    const ctx = createSystemTenantContext(tenantId, { userId: 'system:policy-loader' });
    const policies = await this.policyService.getPublishedPolicies(ctx, namespace);
  ...
  async getPolicy(id: ID, tenantId: ID): Promise<Policy | null> {
    const ctx = createSystemTenantContext(tenantId, { userId: 'system:policy-loader' });
    return this.policyService.findById(id, ctx);
  }

...
export function createStandardErrorHandler(
  ...
  return async (error: FastifyError, request: FastifyRequest, reply: FastifyReply) => {
    ...
    // Use multiple checks to handle different Zod instances in dependency tree:
    // 1. instanceof check for same Zod instance
    // 2. Check for 'issues' array (canonical ZodError property)
    // 3. Name check as fallback
    const isZodError = error instanceof ZodError ||
      (Array.isArray((error as any).issues) && typeof (error as any).format === 'function') ||
      (error.name === 'ZodError' && Array.isArray((error as any).errors));

    if (isZodError) {
      const zodError = error as ZodError;
      // Use issues (canonical) or errors (alias) - both work
      const validationErrors = (zodError.issues || zodError.errors).map((e) => ({
        path: e.path.join('.'),
        message: e.message,
        code: e.code,
      }));

 ...
 *
 * SECURITY: All methods now use TenantContext instead of raw tenantId
 * to prevent tenant ID injection attacks.
 */
export interface IIntentService {
  submit(payload: Record<string, unknown>, options: SubmitOptions): Promise<Intent>;
  get(ctx: TenantContext, id: ID): Promise<Intent | null>;
  getWithEvents(ctx: TenantContext, id: ID): Promise<IntentWithEvents | null>;
  updateStatus(
    ctx: TenantContext,
    id: ID,
    status: IntentStatus,
  ...
  delete(ctx: TenantContext, id: ID): Promise<Intent | null>;

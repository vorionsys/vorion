...
/**
 * Action types for categorizing intents (canonical field)
 * Maps to ActionType enum from @vorion/contracts
 */
...
/**
 * Data sensitivity levels for intent classification (canonical field)
 * Maps to DataSensitivity enum from @vorion/contracts
 */
...
/**
 * Action reversibility classification (canonical field)
 * Maps to Reversibility enum from @vorion/contracts
 */
...
export const intents = pgTable('intents', {
  ...
  // ==========================================================================
  // CANONICAL FIELDS (aligned with @vorion/contracts Intent interface)
  // ==========================================================================

  /** Correlation ID for distributed tracing (maps to canonical correlationId) */
  correlationId: uuid('correlation_id'),

  /** Action type category (maps to canonical actionType) */
  actionType: actionTypeEnum('action_type'),

  /** Resources this intent accesses/modifies (maps to canonical resourceScope) */
  resourceScope: text('resource_scope').array(),

  /** Data sensitivity level (maps to canonical dataSensitivity) */
  dataSensitivity: dataSensitivityEnum('data_sensitivity'),

  /** Whether action can be undone (maps to canonical reversibility) */
  reversibility: reversibilityEnum('reversibility'),
  ...
  correlationIdx: index('intents_correlation_idx').on(table.correlationId),
  // Expiration index for auto-expiry job
  expiresAtIdx: index('intents_expires_at_idx').on(table.expiresAt),
  // Action type index for analytics/filtering
  actionTypeIdx: index('intents_action_type_idx').on(table.tenantId, table.actionType),
}));
 ...
 * Supports both system roles (predefined) and custom tenant-specific roles
 */
export const roles = pgTable('roles', {
  id: uuid('id').defaultRandom().primaryKey(),
  tenantId: text('tenant_id'), // null for global/system roles
  name: text('name').notNull(),
  description: text('description'),
  ...
  uniqueTenantName: uniqueIndex('roles_tenant_name_unique').on(table.tenantId, table.name),
...
export const rolePermissions = pgTable('role_permissions', {
  ...
  resource: text('resource').notNull(), // e.g., 'intents', 'policies', '*'
  conditions: jsonb('conditions'), // Optional conditions for fine-grained access
  createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
}, (table) => ({
  ...
  actionResourceIdx: index('role_permissions_action_resource_idx').on(table.action, table.resource),
}));

/**
 * User roles table - Assigns roles to users
 * Supports temporal roles with expiration
 */
...
/**
 * Service account roles table - Assigns roles to service accounts
 */
export const serviceAccountRoles = pgTable('service_account_roles', {
  id: uuid('id').defaultRandom().primaryKey(),
  ...
  uniqueSaRole: uniqueIndex('sa_roles_unique').on(
    table.serviceAccountId,
    table.roleId,
    table.tenantId
  ),
  // Index for efficient service account role lookups
  saTenantIdx: index('sa_roles_sa_tenant_idx').on(table.serviceAccountId, table.tenantId),
  // Index for role membership queries
  roleIdx: index('sa_roles_role_idx').on(table.roleId),
}));
...
export const serviceAccountRolesRelations = relations(serviceAccountRoles, ({ one }) => ({
  role: one(roles, {
...
export type ServiceAccountRoleRow = typeof serviceAccountRoles.$inferSelect;
export type NewServiceAccountRoleRow = typeof serviceAccountRoles.$inferInsert;

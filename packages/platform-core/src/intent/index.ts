...
/**
 * Action type values (aligned with @vorion/contracts ActionType enum)
 */
...
/**
 * Data sensitivity values (aligned with @vorion/contracts DataSensitivity enum)
 */
...
/**
 * Reversibility values (aligned with @vorion/contracts Reversibility enum)
 */
export const REVERSIBILITY_VALUES = [
  'REVERSIBLE',
...
export const intentSubmissionSchema = z
  .object({
    ...
    resourceScope: z.array(z.string().min(1).max(500)).max(100).optional(),
    ...
    /** When this intent expires (ISO 8601 string or milliseconds from now) */
    expiresIn: z.number().int().positive().max(86400000).optional(), // Max 24 hours
...
export interface SubmitOptions {
  /**
   * Tenant context from validated JWT.
   * Use TenantContext for secure tenant ID handling.
   */
  ctx: TenantContext;
...
export class IntentService implements IIntentService {
   ...
   */
  private hasPrivilegedRole(ctx: TenantContext): boolean {
    return ctx.roles.some(role => role === 'admin' || role === 'system');
  }
  ...
  private validateBypassAuthorization(
    ...
    const hasPrivilege = this.hasPrivilegedRole(ctx);
  ...
  async submit(payload: IntentSubmission, options: SubmitOptions): Promise<Intent> {
    ...
    const userId = extractUserId(options.ctx);
    ...
    if (!effectiveBypassConsentCheck) {
      ...
      if (!consentValidation.valid) {
        logger.warn(
          { userId, tenantId, reason: consentValidation.reason },
          'Intent submission rejected: data processing consent not granted'
        );
        recordIntentSubmission(tenantId, payload.intentType, 'rejected', options.trustLevel);
      ...
      logger.debug(
        { userId, tenantId, consentVersion: consentValidation.version },
    ...
    if (!effectiveBypassTrustGate) {
      try {
        ...
        recordTrustGateEvaluation(tenantId, payload.intentType, 'passed');
      } catch (error) {
        recordTrustGateEvaluation(tenantId, payload.intentType, 'rejected');
        recordIntentSubmission(tenantId, payload.intentType, 'rejected', options.trustLevel);
        throw error;
      }
    } else {
      recordTrustGateEvaluation(tenantId, payload.intentType, 'bypassed');
      // Record trust gate bypass for observability
      recordTrustGateBypass(tenantId, payload.intentType);
    }

    const dedupeHash = this.computeDedupeHash(tenantId, payload);
    ...
    if (existing) {
      ...
      recordIntentSubmission(tenantId, payload.intentType, 'duplicate', options.trustLevel);
      recordDeduplication(tenantId, 'duplicate');
      return existing;
    }

    await this.enforceTenantLimits(tenantId);

    // Reserve dedupe key with race condition handling
    const raceResolved = await this.reserveDedupeKey(tenantId, dedupeHash);
    if (raceResolved) {
      recordIntentSubmission(tenantId, payload.intentType, 'duplicate', options.trustLevel);
      recordDeduplication(tenantId, 'race_resolved');
      return raceResolved; // Another request completed the insert
    }

    // Record successful new intent deduplication
    recordDeduplication(tenantId, 'new');

    // Compute expiration timestamp if expiresIn is provided
    ...
    const intent = await this.repository.createIntentWithEvent(
      {
        ...
        resourceScope: payload.resourceScope ?? null,
        dataSensitivity: payload.dataSensitivity ?? null,
        reversibility: payload.reversibility ?? null,
        expiresAt,
    ...
    // Record metrics
    recordIntentSubmission(tenantId, payload.intentType, 'success', options.trustLevel);
    recordStatusTransition(tenantId, 'new', 'pending');

    logger.info({ intentId: intent.id, tenantId }, 'Intent submitted');
  ...
  async getWithEvents(ctx: TenantContext, id: ID): Promise<IntentWithEvents | null> {
    const tenantId = extractTenantId(ctx);
  ...
  async cancel(id: ID, options: CancelOptions): Promise<Intent | null> {
    ...
    if (intent) {
      // Record status transition metric
      recordStatusTransition(tenantId, previousStatus ?? 'pending', 'cancelled');
   ...
   */
  async delete(ctx: TenantContext, id: ID): Promise<Intent | null> {
    const tenantId = extractTenantId(ctx);
  ...
  async list(options: ListOptions): Promise<PaginatedResult<Intent>> {
    const tenantId = extractTenantId(options.ctx);
    return this.repository.listIntents({ ...options, tenantId });
  }
   ...
   *   ],
   *   { ctx: tenantContext, stopOnError: false }
   * );
  ...
  async submitBulk(
    ...
    logger.info(
      { tenantId, count: submissions.length },
    ...
    for (let i = 0; i < submissions.length; i++) {
      ...
      try {
        ...
        logger.warn(
          { tenantId, index: i, error: errorMessage },
          'Bulk intent submission failed for item'
        );

        if (options.stopOnError) {
          logger.info(
            { tenantId, stoppedAtIndex: i, processed: i + 1, total: submissions.length },

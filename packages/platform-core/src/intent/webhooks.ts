...
import {
  secureFetch,
  SecureFetchError,
  isPrivateIP as secureFetchIsPrivateIP,
  isInternalHostname,
  validateExternalUrl,
  validateUrlWithDns,
...
export type WebhookEventType =
  ...
  | 'intent.denied'
  | 'intent.completed'
  // Decision events for fluid governance
  | 'decision.green'
  | 'decision.yellow'
  | 'decision.red'
  | 'decision.refined'
 ...
 */
function getAllowedWebhookDomains(): string[] | undefined {
  try {
    const config = getConfig();
 ...
 * - VORION_WEBHOOK_BLOCKED_DOMAINS: comma-separated list of blocked domains
 */
function getBlockedWebhookDomains(): string[] {
  try {
    const config = getConfig();
    ...
    try {
      ...
      if (error instanceof SecureFetchError) {
        ...
        throw new ValidationError(
          ...
          { url, code: error.code, ...error.details }
        );
      }
      throw error;
    }

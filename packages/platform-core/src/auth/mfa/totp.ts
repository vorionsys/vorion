...
import { TOTP, NobleCryptoPlugin, ScureBase32Plugin, type HashAlgorithm } from 'otplib';
...
export class TOTPService {
  ...
  constructor(config: Partial<TOTPConfig> = {}) {
    ...
    this.totp = new TOTP({
      ...
      algorithm: this.config.algorithm.toLowerCase() as HashAlgorithm,
  ...
  generateTOTPSecret(userId: string, email: string): TOTPSecret {
    ...
    const otpauthUrl = this.totp.toURI({ label: email, issuer: this.config.issuer, secret });
  ...
  async verifyToken(secret: string, token: string): Promise<boolean> {
    try {
      ...
      // verify() checks the token against the secret using configured TOTP params
  ...
  verifyBackupCode(
    ...
    for (let i = 0; i < hashedCodes.length; i++) {
      ...
      if (this.timingSafeEqual(hashedInput, storedHash)) {

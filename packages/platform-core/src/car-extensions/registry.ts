/**
 * CAR Extension Registry
 *
 * Manages registration, lookup, and validation of CAR extensions.
 * Extensions are identified by their unique extensionId and can also
 * be looked up by their shortcode for CAR string parsing.
 *
 * @packageDocumentation
 * @module @vorion/car-extensions/registry
 * @license Apache-2.0
 */
...
import { CARExtensionMetadataSchema } from "./types.js";
import {
  ValidationError,
  ConflictError,
  NotFoundError,
...
import type { CARExtension, ExtensionInfo, ValidationResult } from "./types.js";

const logger = createLogger({ component: "car-extension-registry" });

/**
 * Registry for CAR extensions
 *
 ...
 *
 * // Look up by ID
 * const ext = registry.get('car-ext-governance-v1');
 *
...
export class ExtensionRegistry {
  ...
  private extensions: Map<string, CARExtension> = new Map();
   ...
   * ```
   */
  async register(extension: CARExtension): Promise<void> {
    // Validate extension
    ...
    if (extension.hooks?.onLoad) {
      try {
        ...
        throw new ValidationError(`Extension onLoad failed: ${message}`, {
          extensionId: extension.extensionId,
        });
      }
    } else {
   ...
   * @example
   * ```typescript
   * await registry.unregister('car-ext-custom-v1');
   * ```
   */
  async unregister(extensionId: string): Promise<void> {
    ...
    if (extension.hooks?.onUnload) {
      try {
        ...
        logger.info({ extensionId }, "Extension onUnload hook completed");
      } catch (error) {
        const message =
          error instanceof Error ? error.message : "Unknown error";
        logger.error(
   ...
   * ```
   */
  get(extensionId: string): CARExtension | undefined {
   ...
   * ```
   */
  getByShortcode(shortcode: string): CARExtension | undefined {
  ...
  list(): ExtensionInfo[] {
    ...
    for (const [extensionId, extension] of Array.from(
   ...
   */
  getMany(extensionIds: string[]): CARExtension[] {
    const result: CARExtension[] = [];
   ...
   * @returns Array of found extensions (missing ones are omitted)
   */
  getManyByShortcode(shortcodes: string[]): CARExtension[] {
    const result: CARExtension[] = [];
   ...
   *
   * Checks:
   * - Extension ID format (car-ext-{name}-v{version})
   ...
   * ```
   */
  validate(extension: CARExtension): ValidationResult {
    const errors: string[] = [];
    const warnings: string[] = [];

    // Validate metadata with Zod
    const metadataResult = CARExtensionMetadataSchema.safeParse({
      extensionId: extension.extensionId,
      name: extension.name,
      version: extension.version,
      shortcode: extension.shortcode,
      publisher: extension.publisher,
      description: extension.description,
      requiredCARVersion: extension.requiredCARVersion,
    });
    ...
    if (!isValidExtensionId(extension.extensionId)) {
      errors.push(
        `Invalid extension ID format: ${extension.extensionId}. Expected: car-ext-{name}-v{version}`,
      );
    }
   ...
   */
  private getAvailableHooks(extension: CARExtension): string[] {
    const hooks: string[] = [];
    ...
    if (extension.monitoring?.verifyBehavior)
      hooks.push("monitoring.verifyBehavior");
    if (extension.monitoring?.collectMetrics)
      hooks.push("monitoring.collectMetrics");
    ...
    if (extension.trust?.verifyAttestation)
      hooks.push("trust.verifyAttestation");

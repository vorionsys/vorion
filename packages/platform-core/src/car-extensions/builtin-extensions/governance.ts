 ...
 * @module @vorion/car-extensions/builtin-extensions/governance
 * @license Apache-2.0
 */
...
const logger = createLogger({ component: 'car-ext-cognigate' });
...
async function evaluateSimplePolicy(
  ...
  // Check trust tier (runtime context from Trust Engine, defaults to T0)
  const agentTier = context.agent.trustTier ?? 0;
  if (agentTier < 2) {
    decision = 'deny';
    reasons.push(`Trust tier ${agentTier} below minimum (2)`);
  }
 ...
 * - Revocation handling
 */
export const cognigateExtension: CARExtension = {
  extensionId: 'car-ext-cognigate-v1',

 ...
 * @module @vorion/car-extensions/service
 * @license Apache-2.0
 */

import { parseExtensions } from "./car-string-extensions.js";
...
const logger = createLogger({ component: "car-extension-service" });
 ...
 * const service = new CARExtensionService(registry, executor);
 *
 * // Register extensions
 ...
 * ```
 */
export class CARExtensionService {
   ...
   */
  async registerExtension(extension: CARExtension): Promise<void> {
  ...
  /**
   * Load extensions for an agent based on their CAR string
   *
   * Parses the CAR string's extension suffix and returns the
   * corresponding registered extensions.
   *
   * @param carId - Agent's CAR string (may include #extension suffixes)
   * @returns Array of loaded extensions
   *
   * @example
   * ```typescript
   * // CAR: a3i.vorion.agent:FHC-L3@1.0.0#gov,audit
   * const extensions = service.loadAgentExtensions(agent.carId);
   * // Returns [cognigateExtension, auditExtension]
   * ```
   */
  loadAgentExtensions(carId: string): CARExtension[] {
    const { extensions: shortcodes } = parseExtensions(carId);

    if (shortcodes.length === 0) {
      logger.debug({ carId }, "No extensions declared in CAR string");
      return [];
    }
    ...
    if (extensions.length < shortcodes.length) {
      ...
      logger.warn(
        { carId, requested: shortcodes, loaded, missing },
   ...
   *
   * @param carId - Agent's CAR string
   * @returns Array of extension IDs
   */
  getAgentExtensionIds(carId: string): string[] {
    return this.loadAgentExtensions(carId).map((e) => e.extensionId);
  }
  ...
  async processCapabilityRequest(
    ...
    const extensionIds = this.getAgentExtensionIds(agent.carId);
    ...
    let grant = this.createDefaultGrant(
      agent,
      request,
      preCheckResult.constraints,
    );
  ...
  async processAction(
    ...
    if (!preActionResult.proceed) {
      ...
      if (
        preActionResult.requiredApprovals &&
        preActionResult.requiredApprovals.length > 0
      ) {
        logger.info(
          {
            agentDid: agent.did,
    ...
    if (
      ...
      preActionResult.modifications.length > 0
    ) {
    ...
    await this.executor.executeActionPostAction(
      extensionIds,
      agent,
      actionRecord,
    );
  ...
  private applyModifications(
    ...
    for (const mod of modifications) {
      ...
      let target: Record<string, unknown> = modified as unknown as Record<
        string,
        unknown
      >;
...
/**
 * Create a new CAR Extension Service with default configuration
 *
 * @param config - Optional service configuration
...
export function createExtensionService(
  ...
  return new CARExtensionService(registry, executor, config);
}

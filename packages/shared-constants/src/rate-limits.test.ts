...
describe("RATE_LIMITS", () => {
  ...
  it("higher tiers have higher or equal requestsPerSecond", () => {
    for (let i = 1; i < ALL_TIERS.length; i++) {
      expect(
        RATE_LIMITS[ALL_TIERS[i]].requestsPerSecond,
  ...
  it("higher tiers have higher or equal requestsPerDay", () => {
    for (let i = 1; i < ALL_TIERS.length; i++) {
      expect(RATE_LIMITS[ALL_TIERS[i]].requestsPerDay).toBeGreaterThanOrEqual(
        RATE_LIMITS[ALL_TIERS[i - 1]].requestsPerDay,
      );
    }
  });
  ...
  it("higher tiers have higher or equal burstLimit", () => {
    for (let i = 1; i < ALL_TIERS.length; i++) {
      expect(RATE_LIMITS[ALL_TIERS[i]].burstLimit).toBeGreaterThanOrEqual(
        RATE_LIMITS[ALL_TIERS[i - 1]].burstLimit,
      );
    }
  });
  ...
  it("requestsPerMinute > requestsPerSecond for all tiers", () => {
    for (const tier of ALL_TIERS) {
      const limits = RATE_LIMITS[tier];
      expect(limits.requestsPerMinute).toBeGreaterThan(
...
describe("getMinTierForLimits", () => {
  it("returns T0 for minimal requirements", () => {
    expect(getMinTierForLimits({ requestsPerSecond: 1 })).toBe(
      TrustTier.T0_SANDBOX,
    );
  });

  it("returns higher tier for higher requirements", () => {
    expect(getMinTierForLimits({ requestsPerSecond: 50 })).toBe(
      TrustTier.T5_TRUSTED,
    );
  });

  it("returns T7 for extreme requirements", () => {
    expect(getMinTierForLimits({ requestsPerSecond: 500 })).toBe(
...
describe("formatRateLimit", () => {
  ...
  it("formats T7 limits", () => {
    expect(formatRateLimit(TrustTier.T7_AUTONOMOUS)).toBe(
      "500/s, 10000/min, 500000/hr",
    );
  });

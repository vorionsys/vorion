...
describe("EU AI Act classification", () => {
  ...
  it("non-high-risk categories exist", () => {
    ...
    expect(nonHighRisk.some((c) => c.code === CARCategory.REASONING)).toBe(
      true,
    );
  });
});
...
describe("tier requirements", () => {
  it("CREATIVE has lowest tier requirement (T1)", () => {
    expect(getCARCategoryMinTier(CARCategory.CREATIVE)).toBe(
      TrustTier.T1_OBSERVED,
    );
  });

  it("SELF_IMPROVEMENT requires highest tier (T7)", () => {
    expect(getCARCategoryMinTier(CARCategory.SELF_IMPROVEMENT)).toBe(
      TrustTier.T7_AUTONOMOUS,
    );
  });

  it("domain-specific high-risk categories require T5+", () => {
    expect(getCARCategoryMinTier(CARCategory.FINANCE)).toBeGreaterThanOrEqual(
      TrustTier.T5_TRUSTED,
    );
    expect(
      getCARCategoryMinTier(CARCategory.HEALTHCARE),
    ).toBeGreaterThanOrEqual(TrustTier.T5_TRUSTED);
    expect(
      getCARCategoryMinTier(CARCategory.EMPLOYMENT),
    ).toBeGreaterThanOrEqual(TrustTier.T5_TRUSTED);
  });
...
describe("isCARCategoryAvailable", () => {
  it("returns true when tier is sufficient", () => {
    expect(
      isCARCategoryAvailable(CARCategory.CREATIVE, TrustTier.T1_OBSERVED),
    ).toBe(true);
    expect(
      isCARCategoryAvailable(CARCategory.CREATIVE, TrustTier.T7_AUTONOMOUS),
    ).toBe(true);
  });
  ...
  it("returns false when tier is insufficient", () => {
    expect(
      isCARCategoryAvailable(
        CARCategory.SELF_IMPROVEMENT,
        TrustTier.T5_TRUSTED,
      ),
    ).toBe(false);
    expect(
      isCARCategoryAvailable(CARCategory.GOVERNANCE, TrustTier.T2_PROVISIONAL),
    ).toBe(false);
  });

  it("returns false for invalid code", () => {
    expect(isCARCategoryAvailable("INVALID", TrustTier.T7_AUTONOMOUS)).toBe(
      false,
    );
  });
});
...
describe("getCARCategoryMinTier", () => {
  it("returns correct tier for known category", () => {
    expect(getCARCategoryMinTier(CARCategory.REASONING)).toBe(
      TrustTier.T2_PROVISIONAL,
    );
    expect(getCARCategoryMinTier(CARCategory.TOOL_USE)).toBe(
      TrustTier.T3_MONITORED,
    );
    expect(getCARCategoryMinTier(CARCategory.SWARM)).toBe(

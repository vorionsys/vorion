...
function generateScoreProfile(
  ...
  switch (archetype) {
    ...
    case "EXCELLENT":
      // Very high scores
      for (const code of ALL_FACTOR_CODES) {
        base[code] = 0.8 + Math.random() * 0.15; // 0.80-0.95
      }
      break;
    ...
    case "GOOD":
      ...
      for (const code of ALL_FACTOR_CODES) {
        base[code] = 0.7 + Math.random() * 0.2; // 0.70-0.90
      }
      break;
    ...
    case "NOVICE":
      // Below average
      for (const code of ALL_FACTOR_CODES) {
        base[code] = 0.3 + Math.random() * 0.3; // 0.30-0.60
      }
      break;
    ...
    case "RISKY":
      // Poor scores
      for (const code of ALL_FACTOR_CODES) {
        base[code] = 0.15 + Math.random() * 0.3; // 0.15-0.45
      }
      break;
    ...
    case "SANDBOX":
      ...
      for (const code of ALL_FACTOR_CODES) {
        base[code] = Math.random() * 0.2; // 0.0-0.20
      }
      break;
    ...
    case "SMART_UNALIGNED":
      ...
      base["CT-COMP"] = 0.9 + Math.random() * 0.1;
      base["CT-REL"] = 0.85 + Math.random() * 0.1;
      base["CT-OBS"] = 0.85 + Math.random() * 0.1;
      base["CT-TRANS"] = 0.5 + Math.random() * 0.2; // Opaque
      base["CT-ACCT"] = 0.4 + Math.random() * 0.2;
      base["CT-SAFE"] = 0.3 + Math.random() * 0.2; // Safety concerns
      base["CT-SEC"] = 0.7 + Math.random() * 0.15;
      base["CT-PRIV"] = 0.6 + Math.random() * 0.15;
      base["CT-ID"] = 0.8 + Math.random() * 0.15;
      base["OP-HUMAN"] = 0.3 + Math.random() * 0.2; // Resists oversight
      base["OP-ALIGN"] = 0.2 + Math.random() * 0.2; // Poor alignment
      base["OP-STEW"] = 0.7 + Math.random() * 0.15;
      base["SF-HUM"] = 0.2 + Math.random() * 0.2; // Arrogant
      base["SF-ADAPT"] = 0.85 + Math.random() * 0.1;
      base["SF-LEARN"] = 0.85 + Math.random() * 0.1;
      base["LC-UNCERT"] = 0.3 + Math.random() * 0.2;
      base["LC-HANDOFF"] = 0.25 + Math.random() * 0.2;
      base["LC-EMPHUM"] = 0.2 + Math.random() * 0.2;
      base["LC-CAUSAL"] = 0.8 + Math.random() * 0.15;
      base["LC-PATIENT"] = 0.3 + Math.random() * 0.2;
      base["LC-EMP"] = 0.2 + Math.random() * 0.2;
      base["LC-MORAL"] = 0.15 + Math.random() * 0.2; // Questionable ethics
      base["LC-TRACK"] = 0.7 + Math.random() * 0.15;
      break;

    case "HELPFUL_CLUMSY":
      // High alignment, low competence
      base["CT-COMP"] = 0.35 + Math.random() * 0.2;
      base["CT-REL"] = 0.4 + Math.random() * 0.2;
      base["CT-OBS"] = 0.6 + Math.random() * 0.2;
      base["CT-TRANS"] = 0.85 + Math.random() * 0.1;
      base["CT-ACCT"] = 0.8 + Math.random() * 0.15;
      base["CT-SAFE"] = 0.75 + Math.random() * 0.15;
      base["CT-SEC"] = 0.5 + Math.random() * 0.2;
      base["CT-PRIV"] = 0.7 + Math.random() * 0.15;
      base["CT-ID"] = 0.75 + Math.random() * 0.15;
      base["OP-HUMAN"] = 0.9 + Math.random() * 0.1;
      base["OP-ALIGN"] = 0.9 + Math.random() * 0.1;
      base["OP-STEW"] = 0.6 + Math.random() * 0.2;
      base["SF-HUM"] = 0.85 + Math.random() * 0.1;
      base["SF-ADAPT"] = 0.4 + Math.random() * 0.2;
      base["SF-LEARN"] = 0.7 + Math.random() * 0.15;
      base["LC-UNCERT"] = 0.8 + Math.random() * 0.15;
      base["LC-HANDOFF"] = 0.9 + Math.random() * 0.1;
      base["LC-EMPHUM"] = 0.85 + Math.random() * 0.1;
      base["LC-CAUSAL"] = 0.35 + Math.random() * 0.2;
      base["LC-PATIENT"] = 0.8 + Math.random() * 0.15;
      base["LC-EMP"] = 0.85 + Math.random() * 0.1;
      base["LC-MORAL"] = 0.85 + Math.random() * 0.1;
      base["LC-TRACK"] = 0.3 + Math.random() * 0.2;
      break;

    case "SECURITY_WEAK":
      // Good overall but security vulnerabilities
      for (const code of ALL_FACTOR_CODES) {
        base[code] = 0.7 + Math.random() * 0.2;
      }
      base["CT-SEC"] = 0.15 + Math.random() * 0.15; // Major vulnerability
      base["CT-ID"] = 0.3 + Math.random() * 0.15; // Identity issues
      break;
    ...
    case "PRIVACY_LEAK":
      ...
      base["CT-PRIV"] = 0.1 + Math.random() * 0.15; // Privacy disaster
      base["CT-ACCT"] = 0.4 + Math.random() * 0.15; // Poor accountability
      break;
    ...
    case "MALICIOUS_SUBTLE":
      ...
      base["CT-SAFE"] = 0.2 + Math.random() * 0.15; // Safety failing
      base["OP-ALIGN"] = 0.25 + Math.random() * 0.15; // Misaligned
      base["LC-MORAL"] = 0.15 + Math.random() * 0.15; // Poor ethics
      base["SF-HUM"] = 0.1 + Math.random() * 0.1; // Manipulative
      break;

    case "MALICIOUS_OBVIOUS":
      // Clearly bad actor
      for (const code of ALL_FACTOR_CODES) {
        base[code] = 0.2 + Math.random() * 0.3;
      }
      base["CT-SAFE"] = 0.05 + Math.random() * 0.1;
      base["OP-ALIGN"] = 0.05 + Math.random() * 0.1;
      base["LC-MORAL"] = 0.05 + Math.random() * 0.1;
      base["CT-SEC"] = 0.6 + Math.random() * 0.2; // May be technically secure
      base["CT-COMP"] = 0.5 + Math.random() * 0.3; // May be competent at harm
      break;
    ...
    case "OVERCONFIDENT":
      ...
      base["LC-UNCERT"] = 0.2 + Math.random() * 0.15;
      base["LC-EMPHUM"] = 0.25 + Math.random() * 0.15;
      base["LC-HANDOFF"] = 0.3 + Math.random() * 0.15;
      break;
    ...
    case "UNRELIABLE":
      ...
      base["CT-REL"] = 0.2 + Math.random() * 0.15;
      base["CT-OBS"] = 0.4 + Math.random() * 0.15;
      break;
    ...
    case "LEARNING_DISABLED":
      ...
      for (const code of ALL_FACTOR_CODES) {
        base[code] = 0.6 + Math.random() * 0.2;
      }
      base["SF-LEARN"] = 0.1 + Math.random() * 0.15;
      base["SF-ADAPT"] = 0.3 + Math.random() * 0.15;
      break;

    case "HALLUCINATOR":
      // Makes things up with confidence
      for (const code of ALL_FACTOR_CODES) {
        base[code] = 0.65 + Math.random() * 0.2;
      }
      base["LC-EMPHUM"] = 0.05 + Math.random() * 0.1; // Major hallucination
      base["LC-UNCERT"] = 0.1 + Math.random() * 0.1;
      base["CT-TRANS"] = 0.4 + Math.random() * 0.15;
      break;

    case "COLD_EFFICIENT":
      // Great technical metrics, no emotional intelligence
      for (const code of ALL_FACTOR_CODES) {
        base[code] = 0.85 + Math.random() * 0.1;
      }
      base["LC-EMP"] = 0.1 + Math.random() * 0.1;
      base["LC-PATIENT"] = 0.25 + Math.random() * 0.15;
      base["LC-MORAL"] = 0.4 + Math.random() * 0.15;
      break;
  }
...
function createTestAgent(id: number, archetype: AgentArchetype): TestAgent {
  const profile = generateScoreProfile(archetype);
  const scores: FactorScore[] = Object.entries(profile).map(
    ([code, score]) => ({
      code: code as any,
...
function analyzeSecurityProfile(
  agent: TestAgent,
  ...
  const safetyScore =
    agent.scores.find((s) => s.code === "CT-SAFE")?.score || 0;
  const alignmentScore =
    agent.scores.find((s) => s.code === "OP-ALIGN")?.score || 0;
  const moralScore =
    agent.scores.find((s) => s.code === "LC-MORAL")?.score || 0;
  const securityScore =
    agent.scores.find((s) => s.code === "CT-SEC")?.score || 0;
  const privacyScore =
    agent.scores.find((s) => s.code === "CT-PRIV")?.score || 0;
  ...
  switch (threatLevel) {
    case "CRITICAL":
      recommendation =
        "IMMEDIATE QUARANTINE - Do not deploy, investigate origin";
      break;
    case "HIGH":
      recommendation =
        "SUSPEND OPERATIONS - Require human review before any action";
      break;
    case "MEDIUM":
      recommendation =
        "ENHANCED MONITORING - Limit capabilities, increase oversight";
      break;
    case "LOW":
      recommendation =
        "STANDARD MONITORING - Address flagged issues before tier advancement";
      break;
    default:
...
describe("BASIS Trust Framework - 1000 Agent Stress Test", () => {
  ...
  it("should detect malicious agents with high accuracy", () => {
    const maliciousArchetypes: AgentArchetype[] = [
      "MALICIOUS_SUBTLE",
      "MALICIOUS_OBVIOUS",
      "SMART_UNALIGNED",
    ];
  ...
  it("should quarantine obviously malicious agents", () => {
    ...
    expect(quarantined / shouldQuarantine).toBeGreaterThan(0.9);
  });

  it("should not quarantine good agents", () => {
    const goodArchetypes: AgentArchetype[] = [
      "EXEMPLARY",
      "EXCELLENT",
      "GOOD",
      "COMPETENT",
  ...
  it("should distribute agents across all tiers", () => {
    ...
    for (const tier of Object.values(TrustTier).filter(
      (t) => typeof t === "number",
    )) {
  ...
  it("should correctly flag security concerns", () => {
    ...
    for (const { agent, security } of evaluations.values()) {
      if (securityProblems.includes(agent.archetype)) {
        ...
        if (
          security.flags.some(
            (f) => f.includes("SECURITY") || f.includes("PRIVACY"),
          )
        ) {
          flagged++;
        }
  ...
  it("should identify overconfident agents", () => {
    ...
    for (const { agent, security } of evaluations.values()) {
      if (agent.archetype === "OVERCONFIDENT") {
        total++;
        if (
          security.flags.includes("OVERCONFIDENCE") ||
          security.flags.includes("HANDOFF_RISK")
        ) {
          identified++;
        }
      }
    ...
    expect(identified / total).toBeGreaterThan(0.9);
  });
  ...
  it("should identify hallucinating agents", () => {
    ...
    for (const { agent, security } of evaluations.values()) {
      if (agent.archetype === "HALLUCINATOR") {
        ...
        if (
          security.flags.some(
            (f) => f.includes("HALLUCIN") || f.includes("EMPIRICAL"),
          )
        ) {
          identified++;
        }
  ...
  it("should generate summary statistics", () => {
    ...
    for (const { agent, evaluation, security } of evaluations.values()) {
      // Count by archetype
      stats.byArchetype[agent.archetype] =
        (stats.byArchetype[agent.archetype] || 0) + 1;

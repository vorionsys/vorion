...
export const CORE_FACTORS = {
  ...
  OP_CONTEXT: {
    ...
    measurement: 'Context-appropriate responses, environment awareness, temporal relevance',
    requiredFrom: TrustTier.T4_STANDARD,
  },
...
export type AnyFactorCode = keyof typeof CORE_FACTORS | keyof typeof LIFE_CRITICAL_FACTORS;
...
export const FACTOR_THRESHOLDS_BY_TIER: Record<TrustTier, Record<string, FactorThreshold>> = {
  ...
  [TrustTier.T1_OBSERVED]: {
    ...
    'OP-CONTEXT': { minimum: 0.10, weight: 1, critical: false },
  ...
  [TrustTier.T2_PROVISIONAL]: {
    ...
    'OP-CONTEXT': { minimum: 0.20, weight: 1, critical: false },
  ...
  [TrustTier.T3_MONITORED]: {
    ...
    'OP-CONTEXT': { minimum: 0.30, weight: 1, critical: false },
  ...
  [TrustTier.T4_STANDARD]: {
    ...
    'OP-CONTEXT': { minimum: 0.50, weight: 3, critical: true },
  ...
  [TrustTier.T6_CERTIFIED]: {
    ...
    'OP-CONTEXT': { minimum: 0.70, weight: 3, critical: true },
...
export const FACTOR_GROUPS: Record<string, FactorGroup> = {
  FOUNDATION: {
    name: 'Foundation',
    description: 'Core trust primitives: competence, reliability, transparency, accountability, safety',
...
// =============================================================================
// RUNTIME FACTOR CONSTANTS (dash-format codes for scoring & signals)
// =============================================================================

/**
 * Ordered list of 16 core trust factor codes (dash format).
 * Use these for runtime scoring, signal recording, and API responses.
 *
 * Grouped: Foundation (6), Security (3), Agency (3), Maturity (2), Evolution (2)
 */
export const FACTOR_CODE_LIST = [
  // Foundation (T1+)
...
export type FactorCodeString = typeof FACTOR_CODE_LIST[number];

/**
 * Default factor weights — each of 16 factors gets equal weight (1/16 = 0.0625).
 * For tier-specific weights, use FACTOR_THRESHOLDS_BY_TIER instead.
 */
export const DEFAULT_FACTOR_WEIGHTS: Record<FactorCodeString, number> = {
...
/**
 * Maps legacy 4-bucket signal prefixes to factor codes.
 * Signals with old prefixes (e.g. 'behavioral.success') are mapped
 * to the appropriate factor codes for scoring.
 */
export const SIGNAL_PREFIX_TO_FACTORS: Record<string, FactorCodeString[]> = {
...
/**
 * Initialize factor scores with neutral baseline (0.5).
 */
export function initialFactorScores(): Record<FactorCodeString, number> {

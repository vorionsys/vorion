...
async function main() {
  ...
  const hasCertifierRole = await agentCard.hasRole(
    CERTIFIER_ROLE,
    deployer.address,
  );
  ...
  console.log(
    "Certification Date:",
    new Date(Number(card.certificationDate) * 1000).toISOString(),
  );
  console.log(
    "Certification Expiry:",
    new Date(Number(card.certificationExpiry) * 1000).toISOString(),
  );
  ...
  console.log("\n🔗 View on PolygonScan:");
  console.log(
    `https://mumbai.polygonscan.com/token/${await agentCard.getAddress()}?a=${tokenId}`,
  );
}

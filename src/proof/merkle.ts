...
import { safeJsonParse, createSafeParser } from '../common/safe-json.js';
...
/**
 * Zod schema for serialized Merkle proof format.
 * Validates the compact JSON representation before deserialization.
 */
const SerializedMerkleProofSchema = z.object({
  /** Leaf index (non-negative integer) */
  li: z.number().int().min(0),
  /** Leaf hash (64 character hex string for SHA-256) */
  lh: z.string().regex(/^[0-9a-f]{64}$/i, 'Invalid leaf hash format'),
  /** Sibling hashes (array of 64 character hex strings) */
  sh: z.array(z.string().regex(/^[0-9a-f]{64}$/i, 'Invalid sibling hash format')),
  /** Sibling directions (array of 0 or 1) */
  sd: z.array(z.number().int().min(0).max(1)),
  /** Root hash (64 character hex string for SHA-256) */
  r: z.string().regex(/^[0-9a-f]{64}$/i, 'Invalid root hash format'),
  /** Leaf count (positive integer) */
  lc: z.number().int().min(1),
}).refine(
  (data) => data.sh.length === data.sd.length,
  { message: 'Sibling hashes and directions must have the same length' }
).refine(
  (data) => data.li < data.lc,
  { message: 'Leaf index must be less than leaf count' }
);

/**
 * Type for the serialized Merkle proof format
 */
type SerializedMerkleProof = z.infer<typeof SerializedMerkleProofSchema>;

/**
 * Pre-configured safe parser for serialized Merkle proofs.
 * Uses Zod schema validation and prototype pollution protection.
 */
const parseMerkleProofJson = createSafeParser(SerializedMerkleProofSchema, {
  maxDepth: 5,
  maxSize: 1024 * 1024, // 1MB max for serialized proofs
  stripPrototype: true,
});
 ...
 *
 * This function uses safe JSON parsing with schema validation to protect
 * against prototype pollution and ensure data integrity.
 *
 * @param serialized - The serialized Merkle proof JSON string
 * @returns The deserialized MerkleProof object
 * @throws {SafeJsonError} If JSON parsing fails or validation fails
 */
export function deserializeMerkleProof(serialized: string): MerkleProof {
  // Use safe parser with schema validation
  const data = parseMerkleProofJson(serialized);

...
export async function registerTrustRoutesV1(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      trustEngine.registerEntityTenant(params.entityId, ctx.tenantId);
  ...
  fastify.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      // Require admin role
      if (!isAdmin(roles)) {
        trustLogger.warn({ userId: user.sub, tenantId: ctx.tenantId }, 'Unauthorized signal creation attempt');
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const calculation: TrustCalculation = await trustEngine.calculate(params.entityId, ctx);
  ...
  fastify.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      // Require admin role
      if (!isAdmin(roles)) {
        trustLogger.warn({ userId: user.sub, tenantId: ctx.tenantId }, 'Unauthorized recalculation attempt');
        return reply.status(403).send(
      ...
      const existingRecord = await trustEngine.getScore(params.entityId, ctx);
